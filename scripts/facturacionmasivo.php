...
if($misesion->verifica_sesion()=="yes")
{
	...
	$sqlRecuperar="SELECT * FROM historia WHERE idhistoria IN (SELECT idhistoria FROM historiacfdi WHERE idfacturacfdi IN (SELECT idfacturacfdi FROM facturacfdi WHERE (pdfok=0 OR pdfok IS NULL) AND txtok=1 AND (cancelada=0 OR cancelada IS NULL) AND idfacturacfdi>=18298)) ORDER BY idcontrato, fechanaturalpago";
