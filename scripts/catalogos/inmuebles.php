...
if($accion=="2")
	{
		...
		while($row = mysql_fetch_array($operacion))
		{
			...
			while($rowdu = mysql_fetch_array($operaciondu))
			{
				...
				$dueniosl .="<tr><td>" . utf8_decode($rowdu["nombre"] . " " . $rowdu["nombre2"] . " " . $rowdu["apaterno"] . " " . $rowdu["amaterno"]) . "</td><td>" . $rowdu["fechacontrato"] . "</td><td><input name='p_" . $rowdu["idduenio"] . "' type='text' value='" . $rowdu["porcentajed"] . "' $isDisabled size='5'></td><td><input type='button' value='X' $isDisabled onClick = \"cargarSeccion('$ruta/controlduenio.php','listaduenio','acumulado=' + acumulado.value + '&dato=" . $rowdu["idd"]  . "&operacion=2')\"></td></tr>";
			
			}
			$dueniosl .="</table><input type='hidden' name='acumulado' id='acumulado' value='$ldu' $isDisabled/>";
	...
	while($row = mysql_fetch_array($datos))
	{
		...
		$html .= "<input type=\"button\" value=\"Borrar\" $isDisabled onClick=\"if(confirm('&iquest;Est&aacute; seguro que desea borrar &eacute;ste elemento?')){cargarSeccion('$ruta/cuentasinmuebles.php','divcuentas','accion=1&idi=$id&id=" .  $row["idcuentainmueble"]  . "' )}\" $txtborrar>";
		$html .= "<input type=\"button\" value=\"Editar\" $isDisabled onClick=\"cargarSeccion('$ruta/cuentasinmuebles.php','divcuentas','accion=2&idi=$id&id=" .  $row["idcuentainmueble"] . "' )\" $txteditar>";
	...
	{
		$dueniosl="<input type='hidden' name='acumulado' id='acumulado' value='' $isDisabled/>";
	}


	$sql="select * from duenio";

	$duenioselect = "<select name=\"duenio\" $isDisabled><option value=\"0\">Seleccione uno de la lista</option>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$paisselect = "<select name=\"idpais\" $isDisabled><option value=\"0\">Seleccione uno de la lista</option>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$tiponimuebleselect = "<select name=\"idtipoinmueble\" $isDisabled><option value=\"0\">Seleccione uno de la lista</option>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$asesorSelect= "<select name=\"idasesor\" $isDisabled>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion)) {
		$seleccionAsesor="";
	...
	if($aud->id!=0)
	{
		$btnaud = "<input type='button' value='Imprimir movimiento' $isDisabled onClick=\"window.open('$ruta/impresiontransaccion.php?id=$aud->id&seccion=Inmueble');\"";
...
<tr>
	...
	<input type="button" value="Aplicar" $isDisabled name="aplicar" $btnblo onClick="if(acumulado.value==''){alert('Debe de tener almenos un dueño, mismo que puede agregar presionando el botón con signo de + que esta a la derecha de la lista');  }else{   if(this.value=='Aplicar'&&priveditar.value==1){ cargarSeccion('$dirscript','contenido','accion=0&id=' + ids.value + '&acumulado=' + acumulado.value + '&calle=' + calle.value + '&numext=' + numext.value + '&numint=' + numint.value + '&colonia=' + colonia.value + '&delmun=' + delmun.value + '&idestado=' + idestado.value + '&idpais=' + idpais.value + '&cp=' + cp.value + '&descripcion=' + descripcion.value + '&inventario=' + inventario.value + '&estacionamiento=' + estacionamiento.value + '&tel=' + tel.value + '&notas=' + notas.value + '&predial=' + predial.value + '&idtipoinmueble=' + idtipoinmueble.value + '&mts2=' + mts2.value + '&idasesor=' + idasesor.value + '&apli=1')};}" >
		...
		<tr>
			...
			<td>
				<input type="text" name="cuentai" value="" $isDisabled>
			</td>
		</tr>
		<!--Cambio 28/06/2021 
		...
		<tr class="condominio" style="display:none;">
			...
			<td>
				<input type="text" id="cond_titular" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">RFC</td>
			<td>
				<input type="text" id="cond_rfc" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Banco</td>
			<td>
				<input type="text" id="cond_banco" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">CLABE</td>
			<td>
				<input type="text" id="cond_clabe" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Referencia</td>
			<td>
				<input type="text" id="cond_ref" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Nombre del administrador</td>
			<td>
				<input type="text" id="cond_admin" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Correo</td>
			<td>
				<input type="text" id="cond_correo" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Teléfono</td>
			<td>
				<input type="text" id="cond_tel" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Días de pago</td>
			<td>
				<input type="text" id="cond_dpago" $isDisabled></input>
			</td>
		</tr>
		<tr class="condominio" style="display:none;">
			<td valign="top">Importe</td>
			<td>
				<input type="text" id="cond_imp" $isDisabled></input>
			</td>
		</tr>
		<!-- Fin Cambio -->
		<tr>
			...
			<td>
				<textarea name="notaci" cols="30" rows="2" $isDisabled></textarea>
			</td>
		</tr>		
		<tr>
...
<tr>
	...
	<td>
		<table border="0">
		<tr>
			<td>Calle</td><td><input type="text" name="calle" value="$calle" $isDisabled size="44"></td>
		<tr>
			<td colspan="2">Num.Ext.<input type="text" name="numext" value="$noext" $isDisabled size="15">&nbsp;&nbsp;
			Num. Int.<input type="text" name="numint" value="$noint" $isDisabled size="15"></td>
		</tr>
		<tr>
			<td>Colonia</td><td><input type="text" name="colonia" value="$colonia" $isDisabled size="44"></td>
		</tr>
		<tr>
			<td>Alc. / Mun.</td><td><input type="text" name="delmun" value="$delnum" $isDisabled size="44"></td>
		</tr>
		<tr>
		...
		<tr>
			<td>C.P.</td><td><input type="text" name="cp" value="$cp" $isDisabled size="44"></td>
		</tr>
		<tr>
			<td>Tama&ntilde;o</td><td><input type="text" name="mts2" value="$mts2" $isDisabled size="15"> Mts<sup>2</sup></td>
		</tr>		
		<tr>
			<td>C. Predial</td><td><input type="text" name="predial" value="$predial" $isDisabled size="44"></td>
		</tr>		
		</table>
	</td>
</tr>
<tr>
	...
	<td>
		...
		<tr>
			<td>
				Descripci&oacute;n
				<br><textarea cols="20" rows="2" name="descripcion" $isDisabled>$descripcion</textarea>
			</td>
			<td>
				Notas
				<br><textarea cols="20" rows="2" name="notas" $isDisabled>$notas</textarea>
			</td>
		</tr>
		<tr>
			<td>
				Inventario
				<br><textarea cols="20" rows="2" name="inventario" $isDisabled>$inventario</textarea>
			</td>
			<td>
				Estacionamiento
				<br><textarea cols="20" rows="2" name="estacionamiento" $isDisabled>$esta</textarea>
			</td>
		</tr>
		<tr>
			<td >Tel&eacute;fono</td><td><input type="text" name="tel" value="$tel" $isDisabled></td>
		</tr>
		</table>
	</td>
</tr>
...
formulario1;
	...
	while($row = mysql_fetch_array($datos))
	{
		...
		$html .= "<td>" .  $row["calle"] . "No." . $row["numeroext"] . " Int." . $row["numeroint"] . " Col." . $row["colonia"] . " Alc/Mun. ". $row["delmun"] . " C.P." . $row["cp"] . " Pa&iacute;s " . $row["pais"] . " Edo " . $row["estado"] . "</td>";
		...
		$html .= "<input type=\"button\" value=\"Ver\" onClick=\"cargarSeccion('$dirscript','contenido','accion=6&id=" .  $row["idinmueble"]  . "' )\" $txteditar>";
