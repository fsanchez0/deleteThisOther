...
if($misesion->verifica_sesion()=="yes")
{
	...
	case "0": // Agrego
		...
		// Cambio 30/07/21
		// Se realiza una consulta para determinar si ya existe un duenio
		// con el mismo CURP
		$sql = "SELECT COUNT(*) as tot FROM duenio WHERE curp = '$curp' GROUP BY curp";
		$result = @mysql_query($sql);
		$row = mysql_fetch_array($result);

		if ( intval($row["tot"])>0){
			echo "<h3>No se cre&oacute; el dueño, ya existe uno con el mismo CURP.</h3>";
			break;
		}
		// Fin Cambio 30/07/21
  ...
  echo "<center><div style=\"background-color:#FA5858; width:400px;\"><strong>Ha ocurrido un error.</strong></div></center>";
		...
		while($row = mysql_fetch_array($operacion))
		{
			...
			while($row1 = mysql_fetch_array($operacion1))
			{
				...
				if($row1["idtipocontacto"] == 1)
				{
					...
					$listat .="<tr><td>" . $row1["contacto"] . "</td><td>" . $row1["notac"] . "</td><td><input type='button' value='X' $isDisabled onClick = \"cargarSeccion('$ruta/contactoduenio.php','teldiv','acumulado=' + tell.value + '&dato=" . $row1["contacto"] . "&operacion=2&tipoc=1')\"></td></tr>";
				
				}
				else
				{
					$maill .="2*" . $row1["contacto"] . "*" . $row1["notac"] . "*" . $row1["usar"] . "|";	
					$listam .="<tr><td>" . $row1["contacto"] . "</td><td>" . $row1["notac"] . "</td><td>" . $row1["usar"] . "</td><td><input type='button' value='X' $isDisabled onClick = \"cargarSeccion('$ruta/contactoduenio.php','maildiv','acumulado=' + maill.value + '&dato=" . $row1["contacto"] . "&operacion=2&tipoc=2')\"></td></tr>";
			...
			$tell = $listat . "<input type='hidden' name='tell' id='tell' value='$tell' $isDisabled/>";
			$maill = $listam . "<input type='hidden' name='maill' id='maill' value='$maill' $isDisabled/>";
	...
	while($rowc = mysql_fetch_array($datosc))
	{
		...
		$html .= "<input type=\"button\" value=\"Borrar\" $isDisabled onClick=\"if(confirm('&iquest;Est&aacute; seguro que desea borrar &eacute;ste elemento?')){cargarSeccion('$ruta/ctasduenio.php','listacuentas','accion=2&id=$id&idc=" .  $rowc["iddueniodistribucion"]  . "' )}\" $txtborrar>";
		$html .= "<input type=\"button\" value=\"Editar\" $isDisabled onClick=\"cargarSeccion('$ruta/lctaduenio.php','ctaform','accion=3&id= $id &idc=" .  $rowc["iddueniodistribucion"]  . "' )\" $txteditar>";
		$html .= "</td></tr>";
	...
	$listac .= "</table>";			
			...
			while($rowdu = mysql_fetch_array($operaciondu))
			{
				...
				$dueniosl .="<tr><td>" . utf8_decode($rowdu["nombre"] . " " . $rowdu["nombre2"] . " " . $rowdu["apaterno"] . " " . $rowdu["amaterno"]) . "</td><td><input name='p_" . $rowdu["idduenio"] . "' type='text' value='" . $rowdu["porcentajed"] . "' size='5'></td><td><input type='button' value='X' $isDisabled onClick = \"cargarSeccion('$ruta/controlduenio.php','listaduenio','acumulado=' + acumulado.value + '&dato=" . $rowdu["idd"]  . "&operacion=2')\"></td></tr>";
	...
	while($row = mysql_fetch_array($datos))
	{
		...
		$listaap .= "<input type=\"button\" value=\"Borrar\" $isDisabled onClick=\"if(confirm('&iquest;Est&aacute; seguro que desea borrar &eacute;ste elemento?')){cargarSeccion('$ruta/apoderados.php','listaap','accion=2&id=$id&idc=" .  $row["idapoderado"]  . "' )}\" $txtborrar>";
		$listaap .= "<input type=\"button\" value=\"Editar\" $isDisabled onClick=\"cargarSeccion('$ruta/apoderados.php','apoderadoform','accion=4&id=$id&idc=" .  $row["idapoderado"]  . "' )\" $txteditar>";
		$listaap .= "</td></tr>";
	...
	if(!$tell or $tell=="")
	{
		$tell="<input type='hidden' name='tell' id='tell' value='' $isDisabled/>";
	}

	if(!$maill or $maill=="")
	{
		$maill="<input type='hidden' name='tmaill' id='maill' value='' $isDisabled/>";
	}
	...
	$agenteselect = "Agente <select name=\"idasesor\"><option value=\"0\" $isDisabled>Seleccione uno de la lista</option>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
...
<tr>
	<td>RFC:</td>
	<td><input type="text" name="rfcd" value="$rfcd" $isDisabled></td>
</tr>
<tr>
	<td>CURP:</td>
	<td><input type="text" name="curp" value="$curp" $isDisabled></td>
</tr>
<!--
<tr>
...
<tr>
	<td>e-mail:</td>
	<td><input type="text" name="emaild" value="$emaild" $isDisabled></td>
</tr>

<tr>
	<td>Porcentaje:</td>
	<td><input type="text" name="porcentaje" value="$porcentaje" $isDisabled></td>
</tr>
-->
<tr>
	<td>Honorarios(%):</td>
	<td><input type="text" name="honorarios" value="$honorarios" $isDisabled></td>
</tr>
<tr>
	<td>I.V.A.(%):</td>
	<td><input type="text" name="iva" value="$iva" $isDisabled></td>
</tr>
<tr>
	<td>Dias a pagar:</td>
	<td><input type="text" name="diasapagar" value="$diasapagar" $isDisabled></td>
</tr>
<tr>
	<td>Usuario:</td>
	<td><input type="text" name="usuario" value="$usuario" $isDisabled></td>
</tr>
<tr>
	<td>Contrase&ntilde;a:</td>
	<td><input type="password" name="pwd" value="$pwd" $isDisabled></td>
</tr>
	</table>
	</fieldset>
	...
	<tr>
		...
		<td>
			<input type="text" name="teln" value="" $isDisabled>
		</td>
	</tr>
		<tr>
		<td valign="top">Notas</td>
		<td>
			<textarea name="notasn" cols="30" rows="2" $isDisabled></textarea>
		</td>
	</tr>
	<tr>
		<td colspan = "2" align = "center">
 		
		<input type="button" value="Agregar" $isDisabled onClick="if(teln.value !=''){cargarSeccion('$ruta/contactoduenio.php','teldiv','acumulado=' + tell.value + '&contacto=' + teln.value  + '&notac=' + notasn .value + '&operacion=1&tipoc=1');}; notasn.value=''; teln.value='';">
		</td>
	</tr>		
	<tr>
	...
	<tr>
		...
		<td>
			<input type="text" name="mailn" value="" $isDisabled>
		</td>
	</tr>
		<tr>
		<td valign="top">Notas</td>
		<td>
			<textarea name="notasm" cols="30" rows="2" $isDisabled></textarea>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<input type="checkbox" name="ocupar" value="" $isDisabled>
		</td>
	</tr>	
	<tr>
		<td colspan = "2" align = "center">
 		
		<input type="button" value="Agregar" $isDisabled onClick="if(mailn.value !=''){cargarSeccion('$ruta/contactoduenio.php','maildiv','acumulado=' + maill.value + '&contacto=' + mailn.value  + '&notac=' + notasm.value + '&ocupar=' + ocupar.checked + '&operacion=1&tipoc=2');};ocupar.checked = false; notasm.value=''; mailn.value='';">
		</td>
	</tr>		
	<tr>
		<td valign="top" align="center" colspan="2">
...
<tr>
	<td>Nombre:</td>
	<td><input type="text" name="nombreap" value="$nombreap" $isDisabled></td>
</tr>
<tr>
	<td>Segundo nombre:</td>
	<td><input type="text" name="nombre2ap" value="$nombre2ap" $isDisabled></td>
</tr>
<tr>
	<td>A. paterno:</td>
	<td><input type="text" name="apaternoap" value="$amaternoap" $isDisabled></td>
</tr>
<tr>
	<td>Ap. materno:</td>
	<td><input type="text" name="amaternoap" value="$apaternoap" $isDisabled></td>
</tr>
<tr>
	<td>Telefono:</td>
	<td><input type="text" name="telap" value="$telap" $isDisabled></td>
</tr>
<tr>
	<td>Correo:</td>
	<td><input type="text" name="mailap" value="$mailap" $isDisabled></td>
</tr>
<tr>
	<td>Facultades:</td>
	<td><textarea name="facultades" cols='30' rows='4' $isDisabled></textarea></td>
</tr>

<tr>
	<td colspan="2" align="center">
	<input type="button" value="Agregar" $isDisabled name="ctabtn" $btncta onClick="if(nombreap.value!='' ){ if(this.value=='Actualizar'&&priveditar.value==1){ cargarSeccion('$ruta/apoderados.php','listaap','accion=' + accionc.value + '&id=' + ids.value + '&nombreap=' + nombreap.value + '&nombre2ap=' + nombre2ap.value + '&apaternoap=' + apaternoap.value + '&amaternoap=' + amaternoap.value +  '&telap=' + telap.value + '&mailap=' + mailap.value   + '&fcultades=' + facultades.value  )};if(this.value=='Agregar'&&privagregar.value==1){ cargarSeccion('$ruta/apoderados.php','listaap','accion=1&id=' + ids.value + '&nombreap=' + nombreap.value + '&nombre2ap=' + nombre2ap.value + '&apaternoap=' + apaternoap.value + '&amaternoap=' + amaternoap.value +  '&telap=' + telap.value + '&mailap=' + mailap.value   + '&facultades=' + facultades.value ); nombreap.value='';nombre2ap.value='';apaternoap.value='';amaternoap.value='';telap.value='';mailap.value='';facultades.value='';}};">
...
<tr>
	<td>Calle:</td>
	<td><input type="text" name="callep" value="$callep" $isDisabled onChange = "called.value=this.value"></td>
</tr>
<tr>
	<td>No. ext.:</td>
	<td><input type="text" name="noexteriorp" value="$noexteriorp" $isDisabled onChange = "noexteriord.value=this.value"></td>
</tr>
<tr>
	<td>No. Int.:</td>
	<td><input type="text" name="nointeriorp" value="$nointeriorp" $isDisabled onChange = "nointeriord.value=this.value"></td>
</tr>
<tr>
	<td>Colonia:</td>
	<td><input type="text" name="coloniap" value="$coloniap" $isDisabled onChange = "coloniad.value=this.value"></td>
</tr>
<tr>
	<td>Alc./Municipio:</td>
	<td><input type="text" name="delmunp" value="$delmunp" $isDisabled onChange = "delmund.value=this.value"></td>
</tr>
<tr>
	<td>Estado:</td>
	<td><input type="text" name="estadop" value="$estadop" $isDisabled onChange = "estadod.value=this.value"></td>
</tr>
<tr>
	<td>Pa&iacute;s:</td>
	<td><input type="text" name="paisp" value="$paisp" $isDisabled onChange = "paisd.value=this.value"></td>
</tr>
<tr>
	<td>C.P.:</td>
	<td><input type="text" name="cpp" value="$cpp" $isDisabled onChange = "cpd.value=this.value"></td>
</tr>
	</table>
	</fieldset>
...
<tr>
	<td>Banco:</td>
	<td><input type="text" name="banco" value="$banco" $isDisabled></td>
</tr>
<tr>
	<td>Titular en el banco:</td>
	<td><input type="text" name="titularbanco" value="$titularbanco" $isDisabled></td>
</tr>
<tr>
	<td>RFC:</td>
	<td><input type="text" name="rfcc" value="$rfcc" $isDisabled></td>
</tr>
<tr>
	<td>Cuenta:</td>
	<td><input type="text" name="cuenta" value="$cuenta" $isDisabled></td>
</tr>
<tr>
	<td>CLABE:</td>
	<td><input type="text" name="clabe" value="$clabe" $isDisabled></td>
</tr>
<tr>
	<td>ID Banorte:</td>
	<td><input type="text" name="idbanco" value="$idbanco" $isDisabled></td>
</tr>
<tr>
	<td>Porcentaje(%):</td>
	<td><input type="text" name="porcentaje" value="$porcentaje" $isDisabled></td>
...
formulario1;
	...
	while($row = mysql_fetch_array($datos))
	{
		...
		$html .= "<input type=\"button\" value=\"Ver\" onClick=\"cargarSeccion('$dirscript','contenido','accion=6&id=" .  $row["idduenio"]  . "' )\" $txteditar>";
