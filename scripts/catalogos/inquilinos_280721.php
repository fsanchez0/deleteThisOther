...
if($misesion->verifica_sesion()=="yes")
{
	...
	case "0": // Agrego
		...
		$verifRFC = "";
			...
			if($tel=="" || $email==""){
				echo "<center><div style='background-color:red;width:400px;color:white;font-size:18px;'>Debe llenar los campos de teléfono y correo electrónico</div></center>";	
			}
			if($filas!=0){
	...
	$estadoselecti = "<select name=\"idestadoi\"><option value=\"0\">Seleccione uno de la lista</option>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$actividadselect = "<select name=\"idactividadeconomica\">";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$giroselect = "<select name=\"idgirocomercial\">";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$nacionalidadselect = "<select name=\"idnacionalidad\"><option value=\"0\">Seleccione uno de la lista</option>";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion))
	{
		$seleccionopt="";
	...
	$usocfdiSelect = "<select name=\"idc_usocfdi\" id=\"idc_usocfdi\">";
	$operacion = mysql_query($sql);
	while($row = mysql_fetch_array($operacion)){
		$seleccionopt="";
...
<tr>
	...
	<td><input type="text" name="fechanacimiento" id="fechanacimiento" value="$fechanacimiento"></td>
...
<tr>
	<td>RFC:</td>
	<td>
		<input type="text" name="rfc" id="rfc" value="$rfc" onChange="aus=this.value;if(aus.length==12){document.getElementById('accionistas').disabled=false;document.getElementById('girocom').style.display='inline';document.getElementById('actividadeco').style.display='none';}else{document.getElementById('accionistas').disabled=true;document.getElementById('girocom').style.display='none';document.getElementById('actividadeco').style.display='inline';};">		
		<input type="button" id="accionistas" value="Accionistas" onClick="cargarSeccion('$ruta/accionistas.php','contenido','idic=$id') ;" $hatilitar >
...
<tr >
	...
	<td><input type="text" name="nombernegocio" value="$nombrenegocio"></td>
</tr>
<tr>
	<td colspan="2">
...
<tr >
	<td>Giro Negocio:</td>
	<td><input type="text" name="gironegocio" value=""></td>
</tr>
-->
...
<tr>
	...
	<td><input type="text" name="email1" value="$email1"></td>
</tr>
<tr>
...
<tr>
	...
	<td><textarea name="direccionf" cols='20' rows='3' >$direccionf</textarea></td>
...
<tr>
	<td colspan='2' align = 'center'>
		...
		<tr>
			<td>Calle:</td>
			<td><input type="text" name="callei" value="$callei"></td>
		</tr>		
		<tr>
			<td>No. Exterior:</td>
			<td><input type="text" name="noexterior" value="$noexterior"></td>
		</tr>
		<tr>
			<td>No. Interior:</td>
			<td><input type="text" name="nointerior" value="$nointerior"></td
		</tr>
		<tr>
			<td>Colonia:</td>
			<td><input type="text" name="coloniai" value="$coloniai"></td
		</tr>	
		<tr>
			<td>C.P.:</td>
		...
		<tr>
			<td>Deleg./Mun.:</td>
			<td><input type="text" name="delmuni" value="$delmuni"></td
		</tr>		
		<tr>
			<td>Localidad:</td>
			<td><input type="text" name="localidad" value="$localidad"></td>
		</tr>
		<tr>
			<td>Referencia:</td>
