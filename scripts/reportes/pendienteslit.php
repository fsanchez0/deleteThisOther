...
if($misesion->verifica_sesion()=="yes")
{
	...
	$sql="select * from submodulo where archivo ='pendienteslit.php'";
	...
	$sql= "SELECT contrato.idcontrato as elidcontrato, inquilino.nombre, inquilino.nombre2, inquilino.apaterno, inquilino.amaterno, inquilino.tel as inqtel,tipocobro, fechagenerado , historia.fechanaturalpago, historia.cantidad, aplicado, historia.interes, historia.iva as ivah, fiador.nombre as fnombre, fiador.nombre2 as fnombre2, fiador.apaterno as fapaterno, fiador.amaterno as famaterno, fiador.direccion as fdireccion, fiador.tel as ftel, calle, numeroext, numeroint, inmueble.colonia, delmun, estado, pais, inmueble.cp, inmueble.tel as itel, inquilino.email as emaili FROM contrato, cobros, inquilino,tipocobro, historia, fiador, inmueble, estado, pais  WHERE cobros.idtipocobro=tipocobro.idtipocobro and contrato.idcontrato=historia.idcontrato and historia.idcobros=cobros.idcobros and contrato.idinquilino=inquilino.idinquilino and historia.aplicado=false and contrato.idfiador=fiador.idfiador and contrato.idinmueble = inmueble.idinmueble and litigio=true and inmueble.idestado = estado.idestado and inmueble.idpais = pais.idpais and historia.fechanaturalpago <= '$hoy' order by inquilino.idinquilino, fechanaturalpago, historia.idhistoria";
	$html = "<h1>Litigios de Inmuebles por cobrar</h1>\n";
	...
	while($row = mysql_fetch_array($operacion))
	{
		...
		if($ccontrato!=$row["elidcontrato"])
		{
			...
			if ($priv[2]=='1')
			{
				...
				$accionlitigio="<input type =\"button\" value=\"A pendientes\" onClick=\"cargarSeccion('$dirscript','contenido','id=" .  $row["elidcontrato"]  . "' )\"  />";
			}
			...
			$Cabeceratabla="<tr><th>Contrato: </th><td colspan=\"2\">" . $row["elidcontrato"]  . " $accionboton&nbsp;&nbsp;$accionlitigio</td></tr>";
			$Cabeceratabla .="<tr><th>Inquilino: </th><td colspan=\"2\">" . $row["nombre"] . " " . $row["nombre2"] . " " . $row["apaterno"] . " " . $row["amaterno"] . "(Tel. " . $row["inqtel"] . ", correo electr&oacute;nico ". $row["emaili"]  .")</td></tr>";
			$Cabeceratabla .="<tr><th>Inmueble: </th><td colspan=\"2\">" .   $row["calle"] . " No." . $row["numeroext"] . " Int." . $row["numeroint"] . " Col." . $row["colonia"] . " Alc/Mun. ". $row["delmun"] . " C.P. " . $row["cp"]  . " Tel. " . $row["itel"] . " </td></tr>";
	...
	$html .= "<table><tr><th>G. T. Renta</th><th>&nbsp;</td><th>G .T. Mantenimiento</th><th>&nbsp;</th><th>G. T. Interes</th><th>&nbsp;</th><th>Gran Total</th></tr>";
