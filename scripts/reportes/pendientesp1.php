...
$hoy2=date('Y-m-d');
	...
	$otros=0;
	$interes=0;
	$html.="<iframe src=\"http://192.168.1.250/scripts/reportes/sumapendiente1.php\" style=\"width:100%;border:0px;height:80px;\"></iframe>";
	$html .= "<table border=\"0\" >";
	$tn=0;
	while($row = mysql_fetch_array($operacion))
	{
		...
		if($ccontrato!=$row["elidcontrato"])
		{
			...
			$filad=$gris;
			$pendiente="window.open( '$ruta/pendientesc1.php?contrato=" . $row["elidcontrato"] . "');";
			//$pendientemail="window.open( '$ruta/pendientesmail.php?contrato=" . $row["elidcontrato"] . "');";
			$pendientemail="mostrarrdivmail('emailenvio',event);cargarSeccion('$ruta/pendientesmailp1.php','emailenvio','contrato=" . $row["elidcontrato"] . "' )";
			...
			$html .=  "\n<tr><td><br><table width=\"596\" border=\"1\"  cellspacing=\"0\" cellpadding=\"5\" bordercolor=\"#FFFFFF\" id=\"tbl_$tn\">$Cabeceratabla<tr><td style=\"$filad\">Fecha nat. pago</td><td style=\"$filad\">Concepto</td><td style=\"$filad\">Cantidad</td><td style=\"$filad\">Acci&oacute;n</td></tr>\n";
