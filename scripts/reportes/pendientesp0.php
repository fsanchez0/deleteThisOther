...
if($misesion->verifica_sesion()=="yes")
{
	...
	while($row = mysql_fetch_array($operacion))
	{
		...
		if($ccontrato!=$row["elidcontrato"])
		{
			...
			$Cabeceratabla .="<tr><th>Inquilino: </th><td colspan=\"3\">" . $row["nombre"] . " " . $row["nombre2"] . " " . $row["apaterno"] . " " . $row["amaterno"] . "(Tel. " . $row["inqtel"] . ", correo electr&oacute;nico ". $row["email"]  . ")</td></tr>";
