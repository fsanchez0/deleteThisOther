...
if($misesion->verifica_sesion()=="yes"){
	...
	$sql="select * from submodulo where archivo ='pendientesSelect.php?ver=0'";
	...
	if($ver==0){
		...
		$sqlSelect = "SELECT  inmueble.idinmueble as idinm, calle, numeroext, numeroint, colonia, delmun, estado, pais, cp FROM contrato, historia, inmueble, estado ,pais  WHERE contrato.idcontrato=historia.idcontrato and historia.aplicado=false and contrato.idinmueble = inmueble.idinmueble and litigio=false and inmueble.idestado = estado.idestado and inmueble.idpais = pais.idpais and historia.fechanaturalpago <='$hoy' group by idinm order by  idinm";
		...
		if (!$result){
			...
			echo '<table id="Table" border="0" aling="center"> <tr><th colspan="2" align="center">REPORTE COBRANZA POR INMUEBLE</th></tr>';

			echo '<tr> <td><table id="ListTable" border="1"> <tr><th colspan="2">Lista Inmuebles</th></tr> <tbody id="TbodyList">';
			echo '<tr><th>ID</th><th>Inmueble</th></tr>';
			while ($row = mysql_fetch_array($result)){
				$idinm=$row["idinm"];
				$direccionInm[$row["idinm"]]=$row["calle"] . "No." . $row["numeroext"] . " Int." . $row["numeroint"] . " Col." . $row["colonia"] . " Alc/Mun. ". $row["delmun"] . " C.P." . $row["cp"];
				$funIntercambio = "intercambioTablas(this.parentNode.parentNode.rowIndex ,'ListTable','TbodySelect')";
				$html = '<tr><td>'.$idinm.'</td><td><p onClick="'.$funIntercambio.' "><font size="2">'.$direccionInm[$idinm].'</font></p></td></tr>';
				echo CambiaAcentosaHTML($html);
				$cont ++;
			}
			echo "</tbody></table></td>";
			$funReporte = "reporteI('SelectTable')";
			echo '<td valign="top"> <table id="SelectTable" border="1" style=background-color:#9C0> <tr><th colspan="2">Inmuebles Seleccionados <input type="button" value="Reporte" onClick="'.$funReporte.'"></th></tr> <tbody id="TbodySelect">';
			echo '<tr><th>ID</th><th>Inmueble</th></tr>';
			echo "</tbody></table></td>";
		...
		if($_REQUEST["descargar"]==true){
			...
			while($row = mysql_fetch_array($operacion)){
				...
				if($ccontrato!=$row["elidcontrato"]){
					...
					if ($priv[2]=='1'){
						...
						$accionlitigio="<input type =\"button\" value=\"A litigio\" onClick=\"cargarSeccion('$ruta/pendientesSelect.php','contenido','ver=0&id=" .  $row["elidcontrato"]  . "' )\"  />";
					}
					...
					$Cabeceratabla .="<tr style=\"$blanco\"><th>Inmueble: </th><td colspan=\"3\">" .   $row["calle"] . " No." . $row["numeroext"] . " Int." . $row["numeroint"] . " Col." . $row["colonia"] . " Alc/Mun. ". $row["delmun"] . " C.P. " . $row["cp"]  . " Tel. " . $row["itel"] . " </td></tr>";
			...
			$html = "<input type=\"button\" value=\"envio masivo\" $habilitado onClick=\"mostrarrdivmail('emailenvio',event);cargarSeccion('$ruta/pendientesmail.php','emailenvio','contrato=' + masivo.value ) \"><input type=\"hidden\" value=\"$masivo\" id=\"masivo\" name=\"masivo\"  \">
				<form action='/scripts/reportes/pendientesSelect.php?ver=1' method='post'>
	 			<input type='hidden' name='descargar' value=true><input type='hidden' name='idinmDes' value=\"$idinmuebles\"><br>
