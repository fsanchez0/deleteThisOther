...
if($misesion->verifica_sesion()=="yes")
{
	...
	$html .= "<table border=\"0\">";
	$tn=0;
	$notas ="<tr><td colspan=\"4\" HEIGHT=100 valign=\"top\" ><b>Notas:</b> </td></tr>";
	while($row = mysql_fetch_array($operacion))
	{
		...
		if($ccontrato!=$row["elidcontrato"])
		{
			...
			$Cabeceratabla .="<tr><th>Obligado Solidario: </th><td colspan=\"3\">" . $row["fnombre"] . " " . $row["fnombre2"] . " " . $row["fapaterno"] . " " . $row["famaterno"]  . " (Tel. " . $row["ftel"] . ", email " . $row["emailf"] . ")</td></tr>";
