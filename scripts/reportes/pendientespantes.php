...
if($misesion->verifica_sesion()=="yes")
{
	...
	while($row = mysql_fetch_array($operacion))
	{
		...
		if($ccontrato!=$row["elidcontrato"])
		{
			...
			//$accionlitigio.="<input type =\"button\" value=\"Descargar Excel\" onClick=\"$excel\"  /> ";
			
			//$accionboton.="<input type =\"button\" value=\"Descargar Excel\" onClick=\"$excel\"/> ";
			//$accionboton.="<input type =\"checkbox\" value=\"". $row["elidcontrato"] . "\" name='c_". $row["elidcontrato"] . "' checked onchange=\"ok=1;if(this.checked==true){ok=1}else{ok=0};actualizamailsp(this.name,ok)\"  />Seleccionar";
			//$botonexcel="<input type='button' value='Descargar Excel' onclick=\"$excel\" style='cursor:pointer;' title='Descargar Libro de Excel'";
	...
	$html = "<input type=\"button\" value=\"envio masivo\" $habilitado onClick=\"mostrarrdivmail('emailenvio',event);cargarSeccion('$ruta/pendientesmail.php','emailenvio','contrato=' + masivo.value ) \"><input type=\"hidden\" value=\"$masivo\" id=\"masivo\" name=\"masivo\"  \"><input type=\"button\" value=\"Descargar Todos\" $habilitado onClick=\"window.open('$ruta/masivoexcel.php?contrato='+masivo.value)\">" . $html;
