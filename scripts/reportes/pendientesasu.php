...
//	$hoy=date('Y') . "-" . date('m') . "-" . date('d');
	...
	while($row = mysql_fetch_array($operacion))
	{
		...
		if($ccontrato!=$row["idasu"])
		{
			...
			$pendiente="window.open( '$ruta/pendientea.php?asunto=" . $row["idasu"] . "');";
			$accionboton="<input type =\"button\" value=\"Ver\" onClick=\"$pendiente\"  />";
			$Cabeceratabla="<tr><th>Asunto: </th><td colspan=\"3\" >" . $row["idasu"]  . "&nbsp; $accionboton</td></tr>";
	...
	$html .= "<table><tr><th align=\"rigth\">Gran Total</th></tr>";
	$html .= "<tr><td align=\"rigth\"><strong>$ " . number_format($grentas,2) . "</strong></td></tr></table>";
	$html .= "</td></tr>";
	$html .= "</table>";
