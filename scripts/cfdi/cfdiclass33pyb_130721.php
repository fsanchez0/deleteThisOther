...
*/
		...
		foreach($this->lconceptos as $d)
		{
			...
			else
			{
                ...
                foreach($this->terceros as $t)
                {
				    ...
				    $ppredial .=
					"<terceros:PorCuentadeTerceros nombre=\"". $t["nombre"] . "\" rfc=\"". $t["rfc"] ."\" version=\"". $t["version"]."\">
	...
	{
		$this->impuestos="";
        //separar los que corresponen a una taza y hacer la suma, cuando este la suma, construir la el traslado
		//echo $this->totalimpuestostrasladados . ":" . $this->totalimpuestosretenidos . ".";
		//echo "Impuestos trasladados </br>";
		//echo $this->totalimpuestostrasladados;
		//echo "Impuestos traslados </br>";
		//foreach($this->impuesto_traslados as $d){
		//	echo $d["impuesto"]."</br>";
		//	echo $d["tasaocuota"]."</br>";
		//}
		...
		if($this->totalimpuestostrasladados>0 || $this->totalimpuestosretenidos>0)
		{
			...
			if($this->totalimpuestostrasladados>0)
			{
				...
				if($iiva16>0)
				{
					$listatras .="<cfdi:Traslado Impuesto=\"002\" TipoFactor=\"Tasa\" TasaOCuota=\"0.160000\" Importe=\"" . $iiva16 . "\"></cfdi:Traslado>"; 
				}
				if($iiva8>0)
				{
					$listatras .="<cfdi:Traslado Impuesto=\"002\" TipoFactor=\"Tasa\" TasaOCuota=\"0.080000\" Importe=\"" . $iiva8 . "\"></cfdi:Traslado>"; 
				}
				
				if($iiva0>0)
				{
					$listatras .="<cfdi:Traslado Impuesto=\"002\" TipoFactor=\"Tasa\" TasaOCuota=\"0.000000\" Importe=\"" . $iiva0 . "\"></cfdi:Traslado>"; 
				}
    ...
    function preparar_ingreso($id,$cat,$row,$folio,$serie,$terceros,$int,$nc)
    {
        ...
        if($terceros==1)
        {
			...
			while($rowt = mysql_fetch_array($operaciont))
			{
			    ...
			    $this->tercero["calle"] =  $this->limpiar($rowt["called"]);
        ...
        $fechaic = strtotime ("-2 hours" , strtotime($fechac));
        $fechanueva = date("Y-m-d",$fechaic) . "T" . date("H:i:s",$fechaic) ;    
        ...
        if($ivap>0)
        {
            ...
            $this->concepto_traslado["importe"] = round((double)$cfdi->concepto_traslado["base"] * (double)$cfdi->concepto_traslado["tasaocuota"] ,2);
	...
	function preparar_duenio($id,$cat,$op,$folio,$serie)
    {
		//$id = id del edoduenio
		//$cat = catalogo de la lista requerida por el PAC.. ya no se usara, era para verificar lista de conceptos
		//$row = recordset de la consulta de la base de datos
        ...
        while($row = mysql_fetch_array($op))
		{
            ...
            {
            //datos receptor
                $sqld = "select * from duenio where idduenio = " . $row["idduenio"];
                $operaciontcal = mysql_query($sqld);
                $rcal =mysql_fetch_array($operaciontcal);

         		$this->receptor_rfc=$this->limpiar($rcal["rfcd"]);
                $this->receptor_nombre=$this->limpiar($rcal["nombre"]) . " " . $this->limpiar($rcal["nombre2"]) . " " . $this->limpiar($rcal["apaterno"]) . " " . $this->limpiar($rcal["amaterno"]);
                $this->receptor_usocfdi="G03"; 
                $r=1;
            }



			$this->concepto["claveprodserv"] = "80131800";
	        $this->concepto["cantidad"] =1;
	        $this->concepto["claveunidad"] ="E48";
	        //$cfdi->concepto["unidad"] ="pieza";

	        $sqlinm = "select * From inmueble where idinmueble = " . $row["idinmueble"];
	        $operacioninm = mysql_query($sqlinm);
	        $rowinm = mysql_fetch_array($operacioninm);

	        $dom = "(" . $rowinm["calle"] . " " . $rowinm["noexterior"] . " - " . $rowinm["nointerior"] . ")"; 


	        $this->concepto["descripcion"] =  $dom . $row["notaedo"];
	        //$this->concepto[$row["notaedo"]"valorunitario"] =number_format( ($cantidadp-$ivap),2,".","");
	        $vunitario = $row["importe"] * -1;

	        $this->concepto["valorunitario"] =number_format( ($vunitario),2,".","");
	        $this->concepto["noidentificacion"] =$row["idcontrato"];
	        $this->concepto["importe"] =$this->concepto["cantidad"]*$this->concepto["valorunitario"];
	        $this->concepto["descuento"] =0;
            ...
            //$this->concepto_traslado["base"] = number_format( ($cantidadp-$ivap),2,".","");
                $this->concepto_traslados = array();
	       		$ivatasa = 0.16;
	            $this->concepto_traslado["base"] = number_format( ($vunitario),2,".","");
	            $this->concepto_traslado["impuesto"] = "002";
	            $this->concepto_traslado["tipofactor"] = "Tasa";
	            $this->concepto_traslado["tasaocuota"] = number_format($ivatasa,6,".","" );
