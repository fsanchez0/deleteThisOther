...
function facturas($idd,$fechag) {
	...
	while($row = mysql_fetch_array($operacion))
	{
		...
		if (mysql_num_rows($operacionr)==0)
		{
			...
			$accionbotonfact="<br>" . '<form name="frm_' . $idf.  '" id="frm_' . $idf .  '" method="POST" action="scripts/reporte2.php" target="trg_' . $idf . '"><input type="hidden" name="idc" id="idc" value ="">'.$inputFechaGen.'<input type="hidden" name="filtro" id="filtro" value =""><input type="hidden" name="archivo" id="archivo" value ="">' . "<input type =\"button\" value=\"Facturar. $idf\" onClick=\"$edocuenta\"  /></form>";	
