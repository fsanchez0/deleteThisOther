...
//consulta para marcados y generados de esta ocación
	...
	$sqlc .=" order by e.idduenio, e.idcontrato,fechaedo,idedoduenio";	
...
//consulta para marcados y generados de esta ocación
  ...
  //PARA LA INSTRUCCION DE LA DISPERSION DE LOS PAGOS
    	...
    	//$pdf->Ln(5);
    	// Cambio 13/07/21
		// Se agregaron las cabeceras para la información que se separo de la descripción
    	$pdf->Cell(70,5,utf8_decode('DESCRIPCIÓN'),1,0,'C',false);
		$pdf->Cell(40,5,utf8_decode('BANCO'),1,0,'C',false);
		$pdf->Cell(40,5,utf8_decode('ID BANCO'),1,0,'C',false);
    	// Fin Cambio 13/07/21
		...
		while($rowc = mysql_fetch_array($operacionc))	
		{
			if ($pdf->GetY() >=240){$pdf->AddPage('P');	}
			// Cambio 13/07/21
			// Se separa la información del banco para que se puede visualizar de forma separada
			// el nombre del banco a donde se realiza el pago, se agrega nueva variable $banco
			// y se quita la información del banco de $descripcion
			$descripcion = $rowc["nombre"]  . " RFC: " .  $rowc["rfc"]  . " Cuenta: " .  $rowc["cuenta"]  . " Clabe: " .  $rowc["clabe"];
			$banco = $rowc["banco"];			
			$idBanco = $rowc["idbanco"];
			// Fin Cambio 13/07/21
			...
			// Cambio 13/07/21
			// Se Cambia el tamanio de 100 a 70 la celda para mostrar la descripcion
			$pdf->MultiCell(70,$dif_y, utf8_decode($descripcion) ,1,'L');//descripcion
			// Fin Cambio 13/07/21
			
			// Cambio 13/07/21
			// Se agrega nueva celda para mostrar el nombre del banco
			$posy2=$pdf->GetY();
    		$posX2=$pdf->GetX();//posicion despues de escribir concepto
			$dif_y = $posy2-$posy1;//obtengo alto de las siguientes celdas¿
			$pdf->SetY($posy1);    		
    		$pdf->SetX(80);//reposiciono Y y X despues del concepto, 10 de margen en x
			$pdf->SetFont('Arial','B',9);
    		$pdf->MultiCell(40,$dif_y, utf8_decode($banco) ,1,'R');//precio
			// Fin Cambio 13/07/21
			
			// Cambio 25/06/21
			// Se agregó una nueva celda para poder visualizar mejor el ID Banco
			$posy2=$pdf->GetY();
    		$posX2=$pdf->GetX();//posicion despues de escribir concepto
			...
			$pdf->SetY($posy1);    		
    		// Cambio 13/07/21
			// Se Cambia el tamanio de 50 a 40 la celda para mostrar la descripcion
			// y la posicion en X para empezar a dibujar la celda		
    		$pdf->SetX(120);//reposiciono Y y X despues del concepto, 10 de margen en x
			$pdf->SetFont('Arial','B',9);			
    		$pdf->MultiCell(40,$dif_y,utf8_decode($idBanco),1,'R');//precio
			// Fin Cambio 13/07/21
			// Fin Cambio
    		...
    		$pdf->SetX(160);//reposiciono Y y X despues del concepto, 10 de margen en x
			//Cambio 25/06/21
			// Se cambia el estilo por aplicar en la siguiente celda
			$pdf->SetFont('Arial','B',9);
    		$pdf->Cell(50,$dif_y,"$ ".number_format($sub, 2, '.', ','),1,1,'R');//precio
    		$dif_y=4;
    		//$pdf->Ln(1);
			//Cambio 25/06/21
			// Se regresa al estilo sin nergitas
			$pdf->SetFont('Arial','',9);
