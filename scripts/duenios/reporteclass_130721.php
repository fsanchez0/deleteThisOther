...
//consulta para marcados y generados de esta ocación
  ...
  //PARA LA INSTRUCCION DE LA DISPERSION DE LOS PAGOS
		...
		while($rowc = mysql_fetch_array($operacionc))	
		{
			...
			$descripcion = $rowc["nombre"]  . " RFC: " .  $rowc["rfc"] . " Banco: " .  $rowc["banco"]  . " Cuenta: " .  $rowc["cuenta"]  . " Clabe: " .  $rowc["clabe"];
			$sub = $totalPagar * ($rowc["porcentaje"]/100);
			$idBanco = " IDBanco: " .  $rowc["idbanco"];
			
			$posy1=$pdf->GetY();//posición antes de escribir concepto
			
			$pdf->MultiCell(100,$dif_y, utf8_decode($descripcion) ,1,'L');//descripcion
			...
			$pdf->SetY($posy1);    		
    		$pdf->SetX(110);//reposiciono Y y X despues del concepto, 10 de margen en x
			$pdf->SetFont('Arial','B',9);
    		$pdf->Cell(50,$dif_y,utf8_decode($idBanco),1,1,'R');//precio
			// Fin Cambio
