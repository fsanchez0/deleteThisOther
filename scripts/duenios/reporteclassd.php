...
class PDF extends PDF_Rotate
{
    ...
    {   
			...
			$this->Cell(100,4,"",0,0,'C');
			$this->Cell(95,4,utf8_decode('Col. San Rafael Alc. Cuauhtémoc C.P. 06470 México D.F.'),0,1,'C');
...

{
	...
	function generaPDF()	
	{
		...
		$pdf->Cell(200,4,utf8_decode("Periodo $periodo."),0,1,'C');
		$pdf->Ln(4);
...
//consulta para marcados y generados de esta ocación
		...
		$sqlc = "select *, d.nombre as nd, d.nombre2 as n2d, d.apaterno as apd, d.amaterno as amd from duenio d, edoduenio e, inmueble i, contrato c, inquilino iq where d.idduenio = e.idduenio and e.idinmueble = i.idinmueble and e.idcontrato = c.idcontrato and c.idinquilino = iq.idinquilino and importe>0  and fechagen='" . $this->fechagen . "'  and reportado = 1 and e.idtipocobro>0  and d.idduenio=" . $this->idduenio . " and traspaso = 0 ";
		...
		while($rowc = mysql_fetch_array($operacionc))	
		{
			...
			if($rowc["idcontrato"] != $idcontrato)
			{
				...
				$pdf->Ln(3);
				$pdf->MultiCell(200,$dif_y, utf8_decode($inmueble ),0 ,1,'L');//descripcion
				$pdf->SetFont('Arial','',7);
				$pdf->Ln(1);
	...
	$sqlc = "select *, d.nombre as nd, d.nombre2 as n2d, d.apaterno as apd, d.amaterno as amd, i.idinmueble as idi from duenio d, edoduenio e, inmueble i where d.idduenio = e.idduenio and e.idinmueble = i.idinmueble  and importe>0  and fechagen='" . $this->fechagen . "'  and reportado = 1 and idcontrato =0 and d.idduenio=" . $this->idduenio . " and traspaso = 0 ";
	...
	$operacionc = mysql_query($sqlc);	
		...
		while($rowc = mysql_fetch_array($operacionc))	
		{
			...
			if($rowc["idi"] != $idi)
			{
				...
				$pdf->SetFont('Arial','B',9);
				$pdf->Ln(3);
				$pdf->MultiCell(200,$dif_y, utf8_decode("Cargos directos" . $inmueble),0 ,1,'L');//descripcion
				$pdf->SetFont('Arial','',7);
				$pdf->Ln(1);
		...
		//notas de credito
		$sqlc = "select *, d.nombre as nd, d.nombre2 as n2d, d.apaterno as apd, d.amaterno as amd from duenio d, edoduenio e where d.idduenio = e.idduenio and importe<>0  and fechagen='" . $this->fechagen . "'  and reportado = 1 and d.idduenio=" . $this->idduenio . " and notacredito = true ";
		...
		while($rowc = mysql_fetch_array($operacionc))	
		{
			...
			if($rowc["idcontrato"] != $idcontrato)
			{
				...
				$pdf->SetFont('Arial','B',9);
				$pdf->Ln(3);
				$pdf->MultiCell(200,$dif_y, utf8_decode("Notas de crédito"),0 ,1,'L');//descripcion
				$pdf->SetFont('Arial','',7);
				$pdf->Ln(1);
			...
			//$pdf->Cell(150,$dif_y, utf8_decode("(Nota de credto) " . $rowc["notaedo"] ) ,1 ,0,'L');//descripcion
...
//---------------------------------------		
//trapasos positivos		
	$sqlc = "select *, d.nombre as nd, d.nombre2 as n2d, d.apaterno as apd, d.amaterno as amd, e.idinmueble as idi from duenio d, edoduenio e where d.idduenio = e.idduenio and e.idinmueble = 0  and  fechagen='" . $this->fechagen . "'  and reportado = 1 and idcontrato =0 and d.idduenio=" . $this->idduenio . " and traspaso = 1 and importe>0";
	$sqlc .= $miwhere . " order by e.idduenio, e.idcontrato,idedoduenio";	
...
//consulta para marcados y generados de esta ocación
	...
	$sqlc = "select *, d.nombre as nd, d.nombre2 as n2d, d.apaterno as apd, d.amaterno as amd, e.idinmueble as idi from duenio d, edoduenio e where d.idduenio = e.idduenio and e.idinmueble = 0  and importe<0  and fechagen='" . $this->fechagen . "'  and reportado = 1 and idcontrato =0 and d.idduenio=" . $this->idduenio . " and traspaso = 0 ";
	$sqlc .= $miwhere . " order by e.idduenio, e.idcontrato,idedoduenio";	
...
/*
    	...
    	//$pdf->Cell(10,5,utf8_decode('IVA'),1,0,'C',false);
    	//$pdf->SetFont('Arial','B',7);
    	//$pdf->Cell(20,5,'PRECIO (c/i)',1,0,'C',false);
    	//$pdf->SetFont('Arial','B',8);    	
    	//$pdf->Cell(20,5,"IMPORTE",1,1,'C',false);
		//$pdf->SetFont('Arial','',8);
		//$pdf->setTextColor(0,0,0);
		$pdf->Ln(4);
...
//consulta para marcados y generados de esta ocación
		...
		$sqlc = "select *, d.nombre as nd, d.nombre2 as n2d, d.apaterno as apd, d.amaterno as amd from duenio d, edoduenio e, inmueble i, contrato c, inquilino iq where d.idduenio = e.idduenio and e.idinmueble = i.idinmueble and e.idcontrato = c.idcontrato and c.idinquilino = iq.idinquilino and importe<>0  and fechagen='" . $this->fechagen . "'  and fechaedo<'$fechamenos'  and   reportado = 0  and d.idduenio=" . $this->idduenio . " ";
		$sqlc .= " order by e.idduenio, e.idcontrato, idtipocobro, idedoduenio";		
...
/*
	...
	if($pdf->GetY()<=140)
	{
    	...
    	$h = 140-($pdf->GetY());
    	//$pdf->Cell(10,$h," ",'LB',0,'C'); //cantidad
    	//$pdf->Cell(10,$h," ",'LB',0,'C'); //unidad
    //$pdf->Cell(100,$h," ",'LB',0,'C'); //desc
    	//$pdf->Cell(20,$h," ",'LB',0,'C'); //unidad
    	$pdf->Cell(80,$h," ",'LB',0,'C'); //desc
    //$pdf->Cell(15,$h," ",'LB',0,'C'); //unidad
    	//$pdf->Cell(10,$h," ",'LB',0,'C'); //unidad
    	$pdf->Cell(20,$h," ",'LB',1,'C');//precio
   	//$pdf->Cell(5,$h," ",'LB',0,'C');//%
   // 	$pdf->Cell(10,$h," ",'LB',0,'C');//iva    
	// 	$pdf->Cell(20,$h," ",'LB',0,'C');//precio
    //	$pdf->Cell(20,$h," ",'LRB',1,'C');
...
/*
    	...
    	$pdf->Cell(115,4,"TOTAL: ",0,0,'R');
    	$pdf->Cell(30,4,"$".number_format($this->subtotal, 2, '.', ','),0,1,'R');
*/
    	//IVA y ejecutivo
    	//$pdf->Cell(10,4,"",0,0,'C');
    	//$pdf->Cell(61,4,"Metodo de pago:" . $this->metododepago,0,0,'L');

    //$pdf->Cell(94,4,"IVA: ",0,0,'R');
    //$pdf->Cell(30,4,"$".number_format($this->totaliva, 2, '.', ','),0,1,'R');

    //cantidad con letra y total

    //$letras=utf8_decode(num2letras($this->total,0,0)." pesos  ");

	//	$this->total = "$".number_format($this->total, 2, '.', ',');

	//	$ultimo = substr (strrchr ($this->total, "."), 1 ); //recupero lo que este despues del decimal

	//	$letras = $letras." ".$ultimo."/100 M. N.";

	//	$pdf->Cell(10,4,"",0,0,'C');
	//	$pdf->Cell(60,4,"Regimen Fiscal: Regimen General de Ley Personas Morales",0,0,'L');
	//	$pdf->SetFont('Arial','B',8);
	//	$pdf->Cell(95,4,"Total: ",0,0,'R');
    //	$pdf->Cell(30,4,$this->total,0,1,'R');


	//	$pdf->Ln(3);
	//	$pdf->SetFont('Arial','B',8);
	//	$pdf->Cell(10,4,"",0,0,'R');
	//	$pdf->Cell(160,4,"Importe en letra: " . strtoupper($letras),0,1,'C');
	//	$pdf->Ln(1);	
...
//*** fin de la segunda hoja		
		...
		switch($this->paraenviar)
		{
		case 0:
    		@$pdf->Output();//muestro el pdf
    		break;
		case 1:
    		$this->pdf= BASE_DIR . "/reporte_" . $this->idduenio . ".pdf"; //cambio para que no envie reportes equivocados
