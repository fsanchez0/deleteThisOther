...
formulario1;
	...
	if($accion==1){
		...
		if($fechai)
		{
			if($fechaf)
			{
				$ff = " AND cd.fechad BETWEEN '$fechai' AND '$fechaf' ";	
				$repok="1";
		...
		$sql = "SELECT cd.seried, cd.foliod, cd.fechad, cd.conceptod, cd.subtotald, cd.impuestos, cd.totald, d.nombre, d.nombre2, d.apaterno, d.amaterno, d.rfcd, d.called, d.noexteriord, d.nointeriord, d.coloniad, d.delmund, d.estadod, d.paisd, d.cpd, fi.fecha, fi.hora, fi.pdfok, fi.txtok FROM facturacfdid fd, facturacfdi fi, cfdiedoduenio cd, duenio d WHERE fd.idcfdiedoduenio=cd.idcfdiedoduenio AND fd.idfacturacfdi=fi.idfacturacfdi AND cd.idduenio=d.idduenio ";
