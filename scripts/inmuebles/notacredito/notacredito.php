...
if($misesion->usuario ==19 || $misesion->usuario ==1|| $misesion->usuario ==70 || $misesion->usuario ==51){
...
//y el metodo para recalcular el efectivo que se va a recibir cuando cambien el valor de alguno de la lista
	while ($row = mysql_fetch_array($result))
	{
		...
		else
		{
			$color = "";
			// Cambio 22/09/21
			// Se aplica un redondeo de precision 2 a la variable suma
			$suma+=round($row["cantidadh"],2);
			// Fin Cambio 22/09/21
		}
		...
		$cantidadh=round($row["cantidadh"],2);
		// Fin Cambio 22/09/21
		if($ultimo==0)
		{
			...
			if ($difdinamica<0)
			{
				...
				$listapagos .= "<tr $color ><td >" . $row["fechanaturalpago"] . "</td><td>" . $concepto . "</td><td>" . ($cantidadh) . "</td><td><input type=\"text\" name=\"c_" . hacernombre($row["idhistoria"], ($cantidadh))  . "\" id=\"" . hacernombre($row["idhistoria"], ($cantidadh))  . "\"  value=\"" . ($difdinamica + $cantidadh ) . "\" onChange=\"compruevasuma('porconfirmar','" . hacernombre($row["idhistoria"], ($cantidadh))  . "', 'totalconfirmar')\"><input type=\"text\" name=\"t_" . hacernombre($row["idhistoria"], ($cantidadh)) . "\" placeholder=\"Descripcion\"></td></tr>";
				// Fin Cambio 22/09/21
				...
				// Cambio 22/09/21
				// Se cambia $row["cantidadh"] por $cantidadh
				$controlesjava.="&t_" . $row["idhistoria"] . "=' + t_" . hacernombre($row["idhistoria"], ($cantidadh)) . ".value +  '&c_" . $row["idhistoria"] . "=' + c_" . hacernombre($row["idhistoria"], ($cantidadh)) . ".value + '";			
				// Fin Cambio 22/09/21
			}
			...
			else
			{
				...
				// Cambio 22/09/21
				// Se cambia $row["cantidadh"] por $cantidadh
				$listapagos .= "<tr $color ><td >" . $row["fechanaturalpago"] . "</td><td>" . $concepto . "</td><td>" . ($cantidadh) . "</td><td><input type=\"text\" name=\"c_" . hacernombre($row["idhistoria"], ($cantidadh))  . "\" id=\"" . hacernombre($row["idhistoria"], ($cantidadh))  . "\" value=\"" . ($cantidadh) . "\" onChange=\"compruevasuma('porconfirmar','" . hacernombre($row["idhistoria"], ($cantidadh))  . "', 'totalconfirmar')\"><input type=\"text\" name=\"t_" . hacernombre($row["idhistoria"], ($cantidadh)) . "\" placeholder=\"Descripcion\"></td></tr>";
				// Fin Cambio 22/09/21
		...
		else
		{
			...
			// Cambio 22/09/21
			// Se cambia $row["cantidadh"] por $cantidadh
			$listapagos .= "<tr $color ><td>" . $row["fechanaturalpago"] . "</td><td>" . $concepto . "</td><td>" . ($cantidadh) . "</td><td><input type=\"text\" name=\"c_" . hacernombre($row["idhistoria"], ($cantidadh))  . "\" id=\"" . hacernombre($row["idhistoria"], ($cantidadh))  . "\" onChange=\"compruevasuma('porconfirmar','" . hacernombre($row["idhistoria"], ($cantidadh))  . "', 'totalconfirmar')\"><input type=\"text\" name=\"t_" . hacernombre($row["idhistoria"], ($cantidadh)) . "\" placeholder=\"Descripcion\"></td></tr>";
			// Fin Cambio 22/09/21
			//$controlesjava.="&c_" . $row["idhistoria"] . "=' + c_" . hacernombre($row["idhistoria"], ($row["cantidadh"] + $row["ivah"])) . ".value +  '&t_" . $row["idhistoria"] . "=' + t_" . hacernombre($row["idhistoria"], ($row["cantidadh"] + $row["ivah"])) . ".value +  '";
			
			// Cambio 22/09/21
			// Se cambia $row["cantidadh"] por $cantidadh
			$controlesjava.="&t_" . $row["idhistoria"] . ($cantidadh). "=' + t_" . hacernombre($row["idhistoria"], ($cantidadh)) . ".value +  '&c_" . $row["idhistoria"] . "=' + c_" . hacernombre($row["idhistoria"], ($cantidadh)) . ".value +'";
			// Fin Cambio 22/09/21
...
if($misesion->usuario==19 || $misesion->usuario==1 || $misesion->usuario==3){
...
// Cambio 22/09/21
// Se elimino de la consulta historia.iva que se le sumaba a
// historia.cantidad, ya que cantidad ya tiene el iva
...
// Cambio 08/09/21	
// Se agrega la variable que va a guardar la nota de la nota de crédito
$valor1 = "";
// Fin Cambio 08/09/21
foreach($_GET as $campo => $valor)
{
	...
	if($dhistoria['0']=='t')
	{
		...
		// Cambio 08/09/21
		// Se comentan las líneas de código que se suponia debían agregar la nota a la historia y 
		// se agrega la asignación del valor a la nueva variable
	...
	if(count($dhistoria)>1)
	{
		...
		while ($row = mysql_fetch_array($result))
		{
			...
			// Cambio 22/09/21
			// Se agrega una nueva variable para almacenar el valor del hcantidad
			// con un redonde de precision 2		
			$hcantidad = round($row["hcantidad"],2);
			// Fin Cambio 22/09/21

			// Cambio 22/09/21
			//Se aplico redondeo con precision de 2 a la variable valor
			$valor = round($valor,2);
			// Fin Cambio 22/09/21
			...
			// Cambio 22/09/21
			// Se cambia $row["hcantidad"] por $hcantidad
			if($hcantidad==$valor)
			// Fin Cambio 22/09/21
			{
				$funcionrecibo="";
				...
				// Cambio 22/09/21
				// Se cambia $row["hcantidad"] por $hcantidad
				$final .="<tr><td>$fechagenerado</td><TD>" . CambiaAcentosaHTML($row["tipocobro"]) . " $addtext (F.N.P. " . CambiaAcentosaHTML($row["fechanaturalpago"])  . ")</td><td>" . ($hcantidad) . "</td><td>Pagado</td><td>";
				// Fin Cambio 22/09/21
...
//if($fechanaturalpago!=0)
				...
				// Cambio 08/09/21
				// Se agrego el valor de la nota en el update
				// Cambio 22/09/21
				// Se quito del query la instruccion que suma iva a canitdad
				$sql1 = "update historia set notacredito = true, aplicado=true, fechapago='" . $fechagenerado . "',  cantidad = cantidad, notas = 'LIQUIDADO CON NOTA DE CREDITO', idmetodopago = $idmetodopago, parcialde = $parcialde,aprobado=1, notanc='".$valor1."' where idhistoria = " . $dhistoria[1];
				// Fin Cambio 22/09/21
				// Fin Cambio 08/09/21
			...
			else//para cuando no cumpla que sean iguales, significa que es menor
			{
				...
				//por pago parcial
					...
					// Cambio 22/09/21
					// Se aplica un redondeo de precision 2 a la variable cantidadh
					$cantidad=round($row0["cantidadh"],2);
					// Fin Cambio 22/09/21
					...
					$sql1 = "insert into historia (idcobros, idcontrato, idusuario, idprioridad, fechagenerado, fechanaturalpago, fechagracia, cantidad, notas, vencido, aplicado,fechavencimiento, interes,condonado,notacredito,idmetodopago,parcialde,aprobado,idcategoriat,iva) values ($idcobro,$idcontrato," . $misesion->usuario . ",$idprioridad,'" . date('Y') . "-" . date('m') . "-" . date('d') . "','$fechanaturalpago', '$fechagracia'," . number_format(($cantidad - $valor),2,".","") . ", 'PENDIENTE', $vencido ,0 ,'$fechavencimiento', $hinteres, false,false,$idmetodopago," . $parcialde .  ",'1',".$row0["idcategoriat"].",".number_format($iva,2,".","").")";
...
//*****					
					...
					// Cambio 08/09/21
					// Se agrego el valor de la nota en el update
					$sql2 = "update historia set cantidad =" . number_format($valor,2,".","") ." , aplicado = true, fechapago='" . $fechagenerado . "', notas = '$estadopago',notacredito = true, idmetodopago=$idmetodopago, parcialde = " . $parcialde . ", aprobado=1, notanc='".$valor1."' where idhistoria = " . $dhistoria[1];
					// Fin Cambio 08/09/21
					/*
...
case 4:
$pend="<tr><th>Id historia</th><th>Contrato</th><th>Inquilino</th><th>Id Cobros</th><th>Fecha Pago</th><th>Cantidad</th><th width=\"300\">Concepto</th><th width=\"300\">Notas NC</th><th>Acciones</th></tr>";
...
while($reg=mysql_fetch_array($sql4)){
	...
	$pend.="<tr><td>".$reg[0]."</td>"."<td>".$reg[1]."</td><td>".$datosinquilino."</td>" ."<td>".$reg[2]."</td>" ."<td>".$fechapagado."</td><td>$".$cantidad."</td>
	<td>".$reg[12]." ".diacompleto($reg[13],0)."</td><td>".$reg[10]."</td>";
	$pend.="<td><input type=\"button\" value=\"Aprobar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=6&idcontrato=$idcontrato&fecharecibio=$fechapagado&idmetodopago= $idmetodopago&cambio=$cambio' + '&idhistoria=$reg[0]'+'&cantidad=$reg[3]');this.disabled =true\" $txteditar> &nbsp;<input type=\"button\" value=\"Cancelar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=7&idcontrato=$idcontrato&fecharecibio=$fechapagado&idhistoria=$reg[0]');this.disabled =true\" $txtborrar></td></tr>";
}
echo <<< listapendiente
...
case 7:
$idhistoria=@$_GET["idhistoria"];
$sql=mysql_query("UPDATE historia SET aprobado=1,cantidadnc=0,fechapago=null,notanc=null WHERE idhistoria='$idhistoria'");
echo <<< canceladatos
...
//insert del ingreso del duenio
		...
		if($tipocobrto=="DEPOSITO")
			$sqling .= "($idd ,$idcon, $idinm, $idtc , '$fechaedod' , '$horaedod', $impedo , false, false, $idhist, 'h_$idhist' , 'NC $notas $tipocobrto en garantia $notanc',0, false  )";
		else
...
//insert del cobro de honorario
		...
		if($tipocobrto=="DEPOSITO")
			$sqlhon .= "($idd ,$idcon, $idinm , $idtc , '$fechaedod' , '$horaedod', $hon , false, false,$idhist, 'h_$idhist' , 'Honorario (" . $row["honorarios"] . "% mas 16% I.V.A) por corretaje de inmueble',$ivah, false  )";
		else
			$sqlhon .= "($idd ,$idcon, $idinm , $idtc , '$fechaedod' , '$horaedod', $hon , false, false,$idhist, 'h_$idhist' , 'Honorario (" . $row["honorarios"] . "% mas 16% I.V.A) de administracion $tipocobrto $mes',$ivah, false  )";
...
*/
		...
		if($tipocobrto=="DEPOSITO")
			$sqling .= "($idd ,$idcon, $idinm, $idtc , '$fechaedod' , '$horaedod', $impedo , false, false,$idhist, 'h_$idhist' , '$notas Nota de credito $tipocobrto en garantia $notanc',0, false ,false )";
		else 
...
?>
------WebKitFormBoundary7WRsxBQsC18B2H4U
