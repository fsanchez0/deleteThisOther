...
<td colspan="4" align="center"><input type="button" value="Aplicar" onClick = "cargarSeccion('$dirscript','contenido', 'paso=3&idcontrato=$idcontrato&efectivo='+efect + '&cambio=' + cambio.value + '$controlesjava');this.disabled =true"> 
...
//if($fechanaturalpago!=0)
				...
				$sql1 = "update historia set notacredito = true, aplicado=true, fechapago='" . $fechagenerado . "',  cantidad = cantidad + iva, notas = 'LIQUIDADO CON NOTA DE CREDITO' where idhistoria = " . $dhistoria[1];
			...
			else//para cuando no cumpla que sean iguales, significa que es menor
			{
				...
				//por pago parcial
					...
					$sql1 = "insert into historia (idcobros, idcontrato, idusuario, idprioridad, fechagenerado, fechanaturalpago, fechagracia, cantidad, notas, vencido, aplicado,fechavencimiento, interes,condonado,notacredito) values ($idcobro,$idcontrato," . $misesion->usuario . ",$idprioridad,'" . date('Y') . "-" . date('m') . "-" . date('d') . "','$fechanaturalpago', '$fechagracia'," . number_format(($cantidad - $valor),2,".","") . ", 'PENDIENTE', $vencido ,0 ,'$fechavencimiento', $hinteres, false,false)";
					$sql3 = "insert into historia (idcobros, idcontrato, idusuario, idprioridad, fechagenerado, fechapago , fechanaturalpago, fechagracia, cantidad, notas, vencido, aplicado,fechavencimiento, interes,condonado, notacredito) values ($idcobro,$idcontrato," . $misesion->usuario . ",$idprioridad,'" . date('Y') . "-" . date('m') . "-" . date('d') . "','" . date('Y') . "-" . date('m') . "-" . date('d') . "','$fechanaturalpago', '$fechagracia'," . (-1 * number_format($valor,2,".","")) . ", 'NOTA DE CREDITO', $vencido ,1 ,'$fechavencimiento', $hinteres, false, true)";
					//echo $sql1 . "<br>";
