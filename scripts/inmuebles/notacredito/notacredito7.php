...
$sql4=mysql_query("SELECT historia.idhistoria,historia.idcontrato,historia.idcobros,historia.cantidadnc,inquilino.nombre,inquilino.nombre2,inquilino.apaterno,inquilino.amaterno,historia.cantidad,historia.iva,historia.notanc,historia.fechapago,tipocobro.tipocobro FROM historia,contrato,inquilino,cobros,tipocobro WHERE historia.aprobado=0 AND historia.idcontrato=contrato.idcontrato AND contrato.idinquilino=inquilino.idinquilino AND historia.idcobros=cobros.idcobros AND cobros.idtipocobro=tipocobro.idtipocobro");$controlesjava="";
while($reg=mysql_fetch_array($sql4)){
	...
	$pend.="<tr><td>".$reg[0]."</td>"."<td>".$reg[1]."<td>".$datosinquilino."</td>" ."</td>"."<td>".$reg[2]."</td>"."<td>$".$cantidad."</td><td>".$reg[12]."</td><td>".$reg[10]."</td>";
	$pend.="<td><input type=\"button\" value=\"Aprobar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=6&idcontrato=$idcontrato&fecharecibio=$fechapagado&efectivo='+efect +'&idmetodopago= $idmetodopago&cambio=$cambio' + '&idhistoria=$reg[0]'+'&cantidad=$reg[3]');this.disabled =true\"> &nbsp;<input type=\"button\" value=\"Cancelar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=7&idcontrato=$idcontrato&fecharecibio=$fechapagado&idhistoria=$reg[0]');this.disabled =true\"></td></tr>";
}
echo <<< listapendiente
