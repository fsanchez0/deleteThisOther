...
//y el metodo para recalcular el efectivo que se va a recibir cuando cambien el valor de alguno de la lista
	...
	$mes=substr($row["fechanaturalpago"],5,2);
	$anio=substr($row["fechanaturalpago"],0,4);
		$concepto .=" ".$mes." ".$anio ;
		if($ultimo==0)
		{
	...
	$anio=substr($row["fechanaturalpago"],0,4);
		$concepto .=" ".$mes." ".$anio ;
			if((string)($row["hiva"] + $row["hcantidad"])==$valor)
			{
				...
				$final .="<tr><td>$fechagenerado</td><TD>" . CambiaAcentosaHTML($row["tipocobro"])  " $addtext (F.N.P. " . CambiaAcentosaHTML($row["fechanaturalpago"])  . ")".$concepto."</td><td>" . ($row["hcantidad"]+$row["hiva"]) . "</td><td>Pagado</td><td>";
...
$sql4=mysql_query("SELECT historia.idhistoria,
                  historia.idcontrato,
                  historia.idcobros,
                  historia.cantidadnc,
                  inquilino.nombre,
                  ...
                  historia.fechapago,
                  tipocobro.tipocobro,
                  historia.fechanaturalpago as fechanaturalpago FROM historia,contrato,inquilino,cobros,tipocobro WHERE historia.aprobado=0 AND historia.idcontrato=contrato.idcontrato AND contrato.idinquilino=inquilino.idinquilino AND historia.idcobros=cobros.idcobros AND cobros.idtipocobro=tipocobro.idtipocobro");
$controlesjava="";
while($reg=mysql_fetch_array($sql4)){
	...
	$mes=substr($reg[13],5,2);
	$anio=substr($reg[13],0,4);
		$concepto .=" ".$mes." ".$anio ;
	$pend.="<tr><td>".$reg[0]."</td>".
	"<td>".$reg[1].
	"<td>".$datosinquilino."</td>" .
	"</td>".
	"<td>".$reg[2]." ".$concepto."</td>".
	"<td>$".$cantidad."</td>
	<td>".$reg[12]."</td><td>".$reg[10]."</td>";
