...
if($misesion->usuario==19 || $misesion->usuario==1){
	$botonguardar="<input type=\"button\" value=\"Aplicar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=3&idcontrato=$idcontrato&efectivo='+efect + '&idmetodopago= $idmetodopago&cambio=' + cambio.value + '$controlesjava');this.disabled =true\">
<input type=\"button\" value=\"Guardar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=5&idcontrato=$idcontrato&efectivo='+efect + '&idmetodopago= $idmetodopago&cambio=' + cambio.value + '$controlesjava');this.disabled =true\">";
}
else{
	$botonguardar="<input type=\"button\" value=\"Guardar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=5&idcontrato=$idcontrato&efectivo='+efect +'&idmetodopago= $idmetodopago&cambio=' + cambio.value + '$controlesjava');this.disabled =true\">";
}
echo <<< paso3
...
//if($fechanaturalpago!=0)
					...
					if(is_null($row["inter"])==true || $row["inter"]==false  )
					{
						...
						if($row["vencido"]==false && ($row["concluido"]==false || is_null($row["concluido"])==true))
						{
							...
							if (mktime(0,0,0,substr($fechagracia, 5, 2),substr($fechagracia, 8, 2),substr($fechagracia, 0, 4)) < mktime(0,0,0,substr($row["fechatermino"], 5, 2),substr($row["fechatermino"], 8, 2),substr($row["fechatermino"], 0, 4)))
							{
								...
								$sql1="insert into historia (idcontrato, idcobros, fechagenerado, fechanaturalpago, fechagracia,cantidad,idusuario,idprioridad,notas,fechavencimiento, iva,condonado,idmetodopago, parcialde,aprobado) ";
								...
								$sql1 .= " values (" . $row["idc"] . ", " . $row["idcob"] . ",'" . $fechagenerado . "','" . $fechanaturalpago . "','" . $fechagracia . "'," . number_format($row["ccantidad"],2,".","") . ", " . $misesion->usuario . "," . $row ["idcprioridad"] . ",'PENDIENTE','" . $fechagracia . "'," . number_format($row["ivac"],2,".","") . ",false,$idmetodopago, $parcialde,'1')";
				...
				$sql1="insert into historia (idcontrato, idcobros, fechagenerado, fechanaturalpago, fechagracia,fechapago, cantidad,idusuario,idprioridad,notas,fechavencimiento,condonado, notacredito, aplicado,aprobado) ";
				$sql1 .= " values (" . $row["idc"] . ", " . $row["idcob"] . ",'" . $fechagenerado . "','" . $row["fechanaturalpago"] . "','" . $fechagenerado . "','" . $fechagenerado . "'," . (-1 * number_format($valor,2,".","")) . ", " . $misesion->usuario . "," . $row ["idcprioridad"] . ",'NOTA DE CREDITO','" . $fechagenerado . "',false,true, true,'1')";
				$result0 = mysql_query ($sql1);			
			...
			else//para cuando no cumpla que sean iguales, significa que es menor
			{
				...
				//por pago parcial
					...
					$sql1 = "insert into historia (idcobros, idcontrato, idusuario, idprioridad, fechagenerado, fechanaturalpago, fechagracia, cantidad, notas, vencido, aplicado,fechavencimiento, interes,condonado,notacredito,idmetodopago,parcialde,aprobado) values ($idcobro,$idcontrato," . $misesion->usuario . ",$idprioridad,'" . date('Y') . "-" . date('m') . "-" . date('d') . "','$fechanaturalpago', '$fechagracia'," . number_format(($cantidad - $valor),2,".","") . ", 'PENDIENTE', $vencido ,0 ,'$fechavencimiento', $hinteres, false,false,$idmetodopago," . $parcialde .  ",'1')";

					$sql3 = "insert into historia (idcobros, idcontrato, idusuario, idprioridad, fechagenerado, fechapago , fechanaturalpago, fechagracia, cantidad, notas, vencido, aplicado,fechavencimiento, interes,condonado, notacredito,idmetodopago,aprobado) values ($idcobro,$idcontrato," . $misesion->usuario . ",$idprioridad,'" . date('Y') . "-" . date('m') . "-" . date('d') . "','" . date('Y') . "-" . date('m') . "-" . date('d') . "','$fechanaturalpago', '$fechagracia'," . (-1 * number_format($valor,2,".","")) . ", 'NOTA DE CREDITO', $vencido ,1 ,'$fechavencimiento', $hinteres, false, true,$idmetodopago,'1')";
...
case 4:
$pend="<tr><th>Id historia</th><th>Contrato</th><th>Inquilino</th><th>Id Cobros</th><th>Cantidad</th><th width=\"300\">Notas NC</th><th>Acciones</th></tr>";
$sql4=mysql_query("SELECT historia.idhistoria,historia.idcontrato,historia.idcobros,historia.cantidadnc,inquilino.nombre,inquilino.nombre2,inquilino.apaterno,inquilino.amaterno,historia.cantidad,historia.iva,historia.notanc FROM historia,contrato,inquilino WHERE historia.aprobado=0 AND historia.idcontrato=contrato.idcontrato AND contrato.idinquilino=inquilino.idinquilino");
$controlesjava="";
while($reg=mysql_fetch_array($sql4)){
	...
	$pend.="<tr><td>".$reg[0]."</td>"."<td>".$reg[1]."<td>".$datosinquilino."</td>" ."</td>"."<td>".$reg[2]."</td>"."<td>$".$cantidad."</td><td>".$reg[10]."</td>";
	$pend.="<td><input type=\"button\" value=\"Aprobar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=6&idcontrato=$idcontrato&efectivo='+efect +'&idmetodopago= $idmetodopago&cambio=$cambio' + '&idhistoria=$reg[0]'+'&cantidad=$reg[3]');this.disabled =true\"> &nbsp;<input type=\"button\" value=\"Cancelar\" onClick =\"cargarSeccion('$dirscript','contenido', 'paso=7&idcontrato=$idcontrato&idhistoria=$reg[0]');this.disabled =true\"></td></tr>";
}
echo <<< listapendiente
...
foreach($_GET as $campo => $valor)
{
		...
		if($regis>0)
	{
		...
		//echo $valor;
		$sql ="update historia set aprobado =0,cantidadnc='$regis',notanc='$valor1' where idhistoria="  . $dhistoria[1];
		$result = @mysql_query ($sql);
...
//if($fechanaturalpago!=0)
				...
				if($row["terceros"]== true && $row["inter"]==false)
				{
					...
					cargaredoduenio($idhistoria, number_format($valor,2,".",""));
				}					
			...
			else//para cuando no cumpla que sean iguales, significa que es menor
			{
				//echo "entre cuando en el campo $campo y el valor no es igual $valor=".($row["ivah"] + $row["hcantidad"]) . "<br>";
				//$sql = "select historia.idhistoria as idhistoriah, historia.idcobros as idcobrosh, historia.idcontrato as idcontratoh, historia.idusuario as idusuarioh, historia.idprioridad as idprioridadh, fechagenerado, fechanaturalpago, fechagracia, fechapago, historia.cantidad as cantidadh, vencido, aplicado, notas, tipocobro,  historia.interes as inter, tipocobro, numero, idmargen, cobros.idcobros as idcob,cobros.cantidad as ccantidad, cobros.idprioridad as idcprioridad,fechavencimiento, historia.interes as hinteres, vencido, cobros.iva as ivac, historia.iva as ivah, fechatermino, concluido from contrato, historia, cobros, tipocobro, periodo where historia.idcobros=cobros.idcobros and cobros.idtipocobro=tipocobro.idtipocobro and cobros.idperiodo=periodo.idperiodo and historia.idcontrato = contrato.idcontrato and idhistoria=" . $dhistoria[1];
				$sql = "select historia.idhistoria as idhistoriah, historia.idcobros as idcobrosh, historia.idcontrato as idcontratoh, historia.idusuario as idusuarioh, historia.idprioridad as idprioridadh, fechagenerado, fechanaturalpago, fechagracia, fechapago, historia.cantidad as cantidadh, vencido, aplicado, notas, tipocobro,  historia.interes as inter, tipocobro, numero, idmargen, cobros.idcobros as idcob,cobros.cantidad as ccantidad, cobros.idprioridad as idcprioridad,fechavencimiento, historia.interes as hinteres, vencido, cobros.iva as ivac, historia.iva as ivah, fechatermino, concluido, terceros from contrato, historia, cobros, tipocobro, periodo, folios where historia.idcobros=cobros.idcobros and cobros.idtipocobro=tipocobro.idtipocobro and cobros.idperiodo=periodo.idperiodo  and historia.idcontrato = contrato.idcontrato and tipocobro.idfolios = folios.idfolios and idhistoria=" . $idhistoria;				
...
case 7:
$idhistoria=@$_GET["idhistoria"];
$sql=mysql_query("UPDATE historia SET aprobado=1,cantidadnc=0 WHERE idhistoria='$idhistoria'");
echo <<< canceladatos
