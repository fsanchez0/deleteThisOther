...
<td>Nombre </td><td colspan="3"><input type="text" name="nombreb" id="nombre"><input type="button" value="Buscar" onClick = "cargarSeccion('$ruta/busqueda2.php','busquedacobro', 'btn=1&dato='+nombreb.value);efect=0;"> </td>
</tr>
<tr>
<td>Inmueble</td><td colspan="3"> <input type="text" name="inmuebleb" id="inmueble"><input type="button" value="Buscar" onClick = "cargarSeccion('$ruta/busqueda2.php','busquedacobro', 'btn=2&dato='+inmuebleb.value);efect=0;"></td>
</tr>
<tr>
<td>No. contrato </td><td colspan="3"><input type="text" name="nocontratob" id="nocontrato"><input type="button" value="Buscar" onClick = "cargarSeccion('$ruta/busqueda2.php','busquedacobro', 'btn=3&dato='+ nocontratob.value);efect=0;"></td>
</tr>
</table>
</form>
...
//************************************** PASO 3 *******************************************************************
case 2: //Muestra los pendientes de pagos que serán afectados
...
$sql = "select idhistoria, historia.cantidad as cantidadh, cobros.cantidad as cantidadc, historia.iva as ivah, calle, numeroext, numeroint,fechanaturalpago, fechagenerado,tipocobro, historia.interes as interesh from historia, cobros, contrato, inmueble, tipocobro where historia.idcontrato=contrato.idcontrato and contrato.idinmueble=inmueble.idinmueble and historia.idcobros=cobros.idcobros and historia.idcontrato=$idc and cobros.idtipocobro=tipocobro.idtipocobro and (aplicado = false or isnull(aplicado)=true) and fechagracia>'" . date("Y")  . "-" . date("m") . "-" . date("d") . "' order by historia.fechagracia, historia.idprioridad";
$result = @mysql_query ($sql);
$norecibos=0;
...
//y el metodo para recalcular el efectivo que se va a recibir cuando cambien el valor de alguno de la lista
	while ($row = mysql_fetch_array($result))
	{
		...
		if($ultimo==0)
		{
			...
			//$difdinamica = $difdinamica - ($row["cantidadh"] + $row["ivah"]);
			$difdinamica = $difdinamica - ($row["cantidadh"]);
			if ($difdinamica<0)
			{
				...
				$listapagos .= "<tr><td>" . $row["fechanaturalpago"] . "</td><td>" . $concepto . "</td><td>" . ($row["cantidadh"] ) . "</td><td><input type=\"text\" name=\"c_" . hacernombre($row["idhistoria"], ($row["cantidadh"] ))  . "\" id=\"" . hacernombre($row["idhistoria"], ($row["cantidadh"]))  . "\"  value=\"" . ($difdinamica + $row["cantidadh"]  ) . "\" onChange=\"compruevasuma('porconfirmar','" . hacernombre($row["idhistoria"], ($row["cantidadh"] ))  . "', 'totalconfirmar')\"></td></tr>";
				...
				$controlesjava.="&c_" . $row["idhistoria"] . "=' + c_" . hacernombre($row["idhistoria"], ($row["cantidadh"] )) . ".value + '";
			}
			...
			else
			{
				...
				$listapagos .= "<tr><td>" . $row["fechanaturalpago"] . "</td><td>" . $concepto . "</td><td>" . ($row["cantidadh"] ) . "</td><td><input type=\"text\" name=\"c_" . hacernombre($row["idhistoria"], ($row["cantidadh"] ))  . "\" id=\"" . hacernombre($row["idhistoria"], ($row["cantidadh"] ))  . "\" value=\"" . ($row["cantidadh"] ) . "\" onChange=\"compruevasuma('porconfirmar','" . hacernombre($row["idhistoria"], ($row["cantidadh"] ))  . "', 'totalconfirmar')\"></td></tr>";
		...
		else
		{
            ...
            $listapagos .= "<tr><td>" . $row["fechanaturalpago"] . "</td><td>" . $concepto . "</td><td>" . ($row["cantidadh"] ) . "</td><td><input type=\"text\" name=\"c_" . hacernombre($row["idhistoria"], ($row["cantidadh"]))  . "\" id=\"" . hacernombre($row["idhistoria"], ($row["cantidadh"] ))  . "\" onChange=\"compruevasuma('porconfirmar','" . hacernombre($row["idhistoria"], ($row["cantidadh"]))  . "', 'totalconfirmar')\"></td></tr>";
			$controlesjava.="&c_" . $row["idhistoria"] . "=' + c_" . hacernombre($row["idhistoria"], ($row["cantidadh"] )) . ".value + '";			
		}
	}
}
...
//if($fechanaturalpago!=0)
					...
					if(is_null($row["inter"])==true || $row["inter"]==false  )
					{
						...
						if($row["vencido"]==false && ($row["concluido"]==false || is_null($row["concluido"])==true))
						{
							...
							if (mktime(0,0,0,substr($fechagracia, 5, 2),substr($fechagracia, 8, 2),substr($fechagracia, 0, 4)) < mktime(0,0,0,substr($row["fechatermino"], 5, 2),substr($row["fechatermino"], 8, 2),substr($row["fechatermino"], 0, 4)))
							{
								...
								//$sql1 .= " values (" . $row["idc"] . ", " . $row["idcob"] . ",'" . $fechagenerado . "','" . $fechanaturalpago . "','" . $fechagracia . "'," . $row["ccantidad"] . ", " . $misesion->usuario . "," . $row ["idcprioridad"] . ",'" . CambiaAcentosaHTML($row["tipocobro"]) . "','" . $fechagracia . "'," . $row["ivac"] . ",false)";
								$sql1 .= " values (" . $row["idc"] . ", " . $row["idcob"] . ",'" . $fechagenerado . "','" . $fechanaturalpago . "','" . $fechagracia . "'," . number_format(($row["ccantidad"] * (1 + ($row["ivac"]/100))),2,".","") . ", " . $misesion->usuario . "," . $row ["idcprioridad"] . ",'PENDIENTE','" . $fechagracia . "'," . number_format($row["ivac"],2,".","") . ",false)";								
								
								//echo "<br><b>ejecuto en completos, siguiente pago</b><br>" . $sql1 . "<br>";
								//eliminado por cmabio de buro
				...
				//$sql1 = "update historia set aplicado = true, fechapago='" . $fechagenerado . "', cantidad = " . $valor . " where idhistoria=" . $dhistoria[1];
			...
			else//para cuando no cumpla que sean iguales, significa que es menor
			{
				...
				//por pago parcial
					...
					//nuevos de cobro
					//$sql1 = "insert into historia (idcobros, idcontrato, idusuario, idprioridad, fechagenerado, fechanaturalpago, fechagracia, cantidad, notas, vencido, aplicado,fechavencimiento, interes,condonado) values ($idcobro,$idcontrato," . $misesion->usuario . ",$idprioridad,'" . date('Y') . "-" . date('m') . "-" . date('d') . "','$fechanaturalpago', '$fechagracia'," . ($cantidad - $valor) . ", 'Diferencia del pago realizado', $vencido ,0 ,'$fechavencimiento', $hinteres, false)";
					...
					$sql2 = "update historia set cantidad = $valor  , aplicado = true, fechapago='" . $fechagenerado . "', notas = '$estadopago' where idhistoria = " . $dhistoria[1];
					//***
					...
					$final .="<tr><td>$fechagenerado</td>" . CambiaAcentosaHTML($row["tipocobro"]) . " $addtext (F.N.P. " . CambiaAcentosaHTML($row["fechanaturalpago"])  . ")</td><td>" . ($row["hcantidad"]) . "</td><td>Parcial</td><td>";
...
$diferencia="$ " . number_format($cambio,2);
	...
	$sqlc="select tipocobro, (b.cantidad * (1+(b.iva/100)) ) suma,(b.interes) elinteres from contrato c, cobros b, tipocobro t where c.idcontrato = b.idcontrato and b.idtipocobro = t.idtipocobro and b.idperiodo<>1 and c.idcontrato = $idcontrato";
...
if($diferencia>=0)
{
	$diferencia = "Cambio de $ $saldo";

}
else
{
	$diferencia = -1 * $saldo;
	$diferencia= "A&uacute;n debe $ $saldo";
}
...
//$enviocorreo->enviar("miguelmp@prodigy.net.mx,lucero_cuevas@prodigy.net.mx,miguel_padilla@nextel.mx.blackberry.com,cemaj@prodigy.net.mx,miguel@padilla-bujalil.com.mx", "Cobro realizado", $mensaje);
$enviocorreo->enviar("miguelmp@prodigy.net.mx,miguel_padilla@nextel.mx.blackberry.com,cemaj@prodigy.net.mx,miguel@padilla-bujalil.com.mx", "Cobro realizado", $mensaje);
