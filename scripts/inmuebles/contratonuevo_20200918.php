...
<tr>
	<td valign="top">
		...
		<tr>
			...
			<td><input type="text" name="fechatermino" value="$fechatermino"></td>
		</tr>
		<tr>
			<td>Observaciones </td>
...
<tr>
	<td colspan="2" align="center"><input type="button" name="siguietne" value="Siguiente >" onClick="if(validafechaphp(fechainicio.value,fechatermino.value)==true){cargarSeccion('$dirscript','contenido','accion=0&paso=2&idc=$idc&inquilino=' + inquilino.value + '&fiador=' + fiador.value + '&inmueble=' + inmueble.value + '&tipocontrato=' + tipocontrato.value + '&fechainicio=' + fechainicio.value + '&fechatermino=' + fechatermino.value + '&deposito=' + deposito.value + '&idasesor=' + idasesor.value + '&observacionesc=' + observacionesc.value)}else{alert('Las fechas de inicio y termino est&aacute;n mal o son incoerentes')}"></td>
</tr>
</table>
...
case 2: // paso para la asignacin de cobros
	...
	if(!$idc)
	{
		$sql = "insert into contrato (idusuario, idtipocontrato, idinmueble, idinquilino, idfiador, fechainicio, fechatermino, deposito, concluido, litigio, enedicion,activo,idasesor, observacionesc) values ($idusuario,$tipocontrato, $inmueble, $inquilino, $fiador, '$fechainicio', '$fechatermino', $deposito, false,false, true,true,$idasesor,'$observacionesc')";
...
<tr>
	<td valign="top">
		...
		<tr>
			<td>IVA <input type="text" name="porcentaje" id="porcentaje" size="2" onChange="iva.value=(this.value/100)*cantidad.value;"/>%</td>
			...
			if($cantidadapa>0)
			{
				...
				if($aux>0)
				{
					...
					if($row["idperiodo"]!=1)
					{
						...
						$final .="<tr><td>$fechagenerado</td>" . CambiaAcentosaHTML($row["tipocobro"]) . " </td><td>" . ($row["cantidad"] + $row["iva"] ) . "</td><td>Pagado</td><td>";
						$final .= '<p><input type="button" value="' . CambiaAcentosaHTML($row["tipocobro"]) . $addtext . ' (Recibo F)" onClick="' . $funcionrecibo . '"> </p>';
						$final .="</td></tr>";
						$final2 .="<tr><td>$fechagenerado</td>" . CambiaAcentosaHTML($row["tipocobro"]) . " </td><td>" . ($row["cantidad"] + $row["iva"] ) . "</td><td>Pagado</td><td>";
...
//para los mantenimentos seleccionados
		...
		if($cantidadapa>0)
		{
			...
			$sql = "update apartado set cantidad =" . number_format($cantidadapa,2,".","") . " , cancelado = true where idapartado = $idapa";
			$operacion = mysql_query($sql);
		...
		else
		{
			...
			$sql = "update apartado set devolucion=true, cancelado = true where idapartado = $idapa";
			$operacion = mysql_query($sql);
			
		}
		$edocuenta="window.open( '$ruta/edocuenta.php?contrato=$idc');";
		$accionboton="<input type =\"button\" value=\"Ver\" onClick=\"$edocuenta\"  />";
		echo "<br> Contrato $idc Generado satisfactoriamente $adicional$accionboton<br><br>$final";
