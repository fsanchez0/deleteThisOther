...
// Cambio 10/08/21
// Se agrega variable para poder manejar un nuevo 
// parametro en la solicitud GET
...
// Cambio 10/08/21
// Se crea una nueva variable que nos permite utilizar el valor del tipo de facturacion
// durante varios pasos del proceso, además se agrega un areglo que contiene los tipos 
// de facturacion que se van a manejar
$facturacionPrevia=0;
$tipofacturacionArray = array(array("tipofactura"=>"Facturacion al cubrir pago","idtipofactura"=>0),array("tipofactura"=>"Facturacion previa","idtipofactura"=>1));
// Fin Cambio 10/08/21
...
case 1: // para crear el cuestionario
	...
	if($accion=="1")
	{
		...
		$operacion = mysql_query($sql);
		extract( mysql_fetch_array($operacion));		
		$direccion="$calle No. $numeroext Int. $numeroint Col. $colonia Deleg. $delmun C.P. $cp";
	...
	// Cambio 10/08/21
	// Se agrega una seccion de codigo que nos permite crear la lista de opciones de tipo de facturacion
	// al momento de crear la factura
	$tipofacturacionselect= "<select name=\"tipofacturacion\">";;
	foreach($tipofacturacionArray as $row)
	{
		$seleccionopt="";
		if (@$facturaprevia==$row["idtipofactura"])
		{
			$seleccionopt=" SELECTED ";
		}
		$tipofacturacionselect .= "<option value=\"" . $row["idtipofactura"] . "\" $seleccionopt>" . $row["tipofactura"] . "</option>";

	}
	$tipofacturacionselect .="</select>";
	// Fin Cambio 10/08/21
...
case 2: // paso para la asignacin de cobros
	...
	{
	    // Cambio 10/08/21
		// Se modifica la consulta para que en la inserción se 
		// agregue el valor de facturaprevia
	    $sql = "insert into contrato (idusuario, idtipocontrato, idinmueble, idinquilino, idfiador, fechainicio, fechatermino, deposito, concluido, litigio, enedicion,activo,idasesor,idapartado, observacionesc, facturaprevia) values ($idusuario,$tipocontrato, $inmueble, $inquilino, $fiador, '$fechainicio', '$fechatermino', $deposito, false,false, true,true,$idasesor,$apartado,'$observacionesc',$tipofacturacion)";
		// Fin Cambio 10/08/21
	...
	if($accion=="0") // para mostrar lo que se captur
	{
		...
		while($row = mysql_fetch_array($operacion))
		{
			if($inicial==0)//para asignar los valores de las variables para el formulario
			{
				...
				$inmueble=$row["calle"] . " No. " . $row["numeroext"]  . " Int. " . $row["numeroint"] . " Col. " . $row["colonia"]  . " Del/Mun. " . $row["delmun"]  . " C.P. " . $row["cp"];		
				...
				// Cambio 10/08/21
				// Se modifican los valores de las variables para almacenar el tipo de facturación que se maneja
				// en el contrato en cuestion
				$tipofacturacion=$tipofacturacionArray[intval($row["facturaprevia"])]["tipofactura"];
...
<tr>
	<td ><b>Tipo Facturacion</b></td>
	<td>$tipofacturacion</td>
</tr>
<tr>
...
paso3;
	...
	else//para aplicar el contrato en el historial y cambiar de en edicion a falso
	{
		...
		$sql="SELECT * FROM cobros, periodo, tipocobro, contrato WHERE cobros.idtipocobro = tipocobro.idtipocobro AND cobros.idperiodo = periodo.idperiodo AND contrato.idcontrato = cobros.idcontrato AND contrato.idcontrato = $idc ORDER BY cobros.idprioridad";
		// Fin Cambio 10/08/21
		$operacion = mysql_query($sql);
		while($row = mysql_fetch_array($operacion))
		{
			...
			if($cantidadapa>0)
			{
				...
				if($aux>0)
				{
					...
					$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, fechapago, cantidad, iva, vencido, aplicado, interes, condonado,notas,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechagenerado . "'," .  number_format(($row["cantidad"] * (1 + ($row["iva"]/100)) ), 2, '.', '')  . "," .  number_format($row["iva"], 2, '.', '') . ",false, true ,false,false,'Aplicado del apartado',6,".$row["idcategoriat"].")";
					$va=1;
					...
					if($row["idperiodo"]!=1)
					{
						...
						$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, cantidad, iva, vencido, aplicado, interes, condonado,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechanaturalpago . "','" . $fechagracia . "','" . $fechagracia . "'," . number_format(($row["cantidad"] * (1 + ($row["iva"]/100)) ), 2, '.', '') . "," . number_format($row["iva"], 2, '.', '')  . ",false, false,false,false,6,".$row["idcategoriat"].")";
					}
				}
				else
				{
					if($aux ==0)
					{
						...
						$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, fechapago, cantidad, iva, vencido, aplicado, interes, condonado,notas,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "'," . number_format(($row["cantidad"] * (1 + ($row["iva"]/100)) ), 2, '.', '')  . "," . number_format($row["iva"], 2, '.', '')  . ",false, true ,false,false,'Aplicado del apartado',6,".$row["idcategoriat"].")";
						$va=1;
						if($row["idperiodo"]!=1)
						{
							//echo "<br>$sql1";
						...
						$final2 .="</td></tr>";
							...
							$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, cantidad, iva, vencido, aplicado, interes, condonado,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechanaturalpago . "','" . $fechagracia . "','" . $fechagracia . "'," . number_format(($row["cantidad"] * (1 + ($row["iva"]/100)) ), 2, '.', '') . "," . $row["iva"] . ",false, false,false,false,6,".$row["idcategoriat"].")";
						}
					...
					else
					{
						...
						$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, fechapago, cantidad, iva, vencido, aplicado, interes, condonado,notas,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "'," . number_format( $cantidadapa, 2, '.', '') . "," . number_format($row["iva"], 2, '.', '') . ",false, true ,false,false,'Aplicado del apartado',6,".$row["idcategoriat"].")";
						//echo "<br>$sql1";
						$operacion1 = mysql_query($sql1);
						...
						$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, cantidad, iva, vencido, aplicado, interes, condonado,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "'," . number_format($aux, 2, '.', '') . ",0,false, false ,false,false,6,".$row["idcategoriat"].")";
						$cantidadapa = 0;
					}
			...
			else
			{
				...
				$sql1="insert into historia (idcobros, idcontrato, idprioridad, idusuario, fechagenerado, fechanaturalpago, fechagracia, fechavencimiento, cantidad, iva, vencido, aplicado, interes, condonado,idmetodopago,idcategoriat) values (" . $row["idcobros"] . ",$idc, " . $row["idprioridad"] . ",$idusuario,'" . $fechagenerado . "','" . $fechainicio . "','" . $fechainicio . "','" . $fechainicio . "'," . number_format(($row["cantidad"] * (1 + ($row["iva"]/100)) ), 2, '.', '') . "," . number_format($row["iva"], 2, '.', '') . ",false, false,false,false,6,".$row["idcategoriat"].")";
...
//		$lista .= "<ol>";
		...
		while ($fechaa < $fechat )
		{
            ...
            $sqlh2="insert into historia (idcontrato,idcobros,fechagenerado,fechanaturalpago,fechagracia,cantidad,iva,interes,vencido,aplicado, idusuario,idprioridad,fechavencimiento,condonado,idmetodopago,idcategoriat) values(";			
			$sqlh2 .= $idco . "," . $idcc . ",'" . $HoyFecha . "', '" . $fechaa  . "', '" . $ProxVencimiento . "'," .  number_format(($row["cantidad"] * (1 + ($row["iva"]/100)) ), 2, '.', '') . "," . number_format($rowh["ivac"],2,".","") . ",false,false,false," . $misesion->usuario . "," . $rowh["idprioridadc"] . " , '" . $ProxVencimiento . "',false,6,".$rowh["idcategoriat"].");";
			//echo "<br>";
