...
*/
	...
	echo "<p>Vencimientos:<table border=1 id=\"cierretbl\"><tr><th>Contrato</th><th>Nombre</th><th>Inmueble</th><th>Interes</th><th>Adeudo</th><th>A pagar*</th></tr>";
	...
	while ($row = @mysql_fetch_array($result))
	{
		...
		//aplicar las penalizaciones correspondientes segun el cobro programado
			...
			if($ver==1){ //si confirmaron se hacen los procesos: $ver=1, es la confirmación del cierre
				...
				if(($row["concluido"]==false || is_null($row["concluido"])==true) && $fechanaturalpago!=0)
				{
					//agrega otro interes					
					...
					$result2 = @mysql_query ($sql2);
					//actualiza la fecha de vencimiento para la generación del interes del siguiente mes.
					...
					if($fechanaturalpago!=$fechanpago)//verifica el inmediato
					{
						if($row["concluido"]==false || is_null($row["concluido"])==true)
						{
							...
							if (mktime(0,0,0,substr($ProxVencimiento, 5, 2),substr($ProxVencimiento, 8, 2),substr($ProxVencimiento, 0, 4)) < mktime(0,0,0,substr($row["fechatermino"], 5, 2),substr($row["fechatermino"], 8, 2),substr($row["fechatermino"], 0, 4)))
							{
								...
								$sql2 .= $row["idcontra"] . "," . $row["idcobr"] . ",'" . $HoyFecha . "', '" . $fechanaturalpago . "', '" . $ProxVencimiento . "'," . number_format($row["cantidadc"],2,".","") . "," . number_format($row["ivac"],2,".","") . ",false,false,false," . $misesion->usuario . ", " .$row["idprioridadc"] . " , '" . $ProxVencimiento . "',false)";
								$result2 = @mysql_query ($sql2);
			...
			if($row["concluido"]==false || is_null($row["concluido"])==true)
			{
				...
				$html = "<tr><td>" . $row["idcontra"] . "</td><td>" . $row["nombrei"] . " "  . $row["nombre2"] . " " . $row["apaterno"] . " " . $row["amaterno"] . "</td><td>" . $row["calle"] . " " .$row["numeroext"] . " " . $row["numeroint"]  . "</td><td>" . $row["cinteres"] . "<br></td><td>" . $row["cantida"] . "</td><td>$TotalPagar</td><td>$accionboton</td></tr>";
	...
	{
		echo "</table>";
		$mensaje = "Se ha realizado el cierre manual a las " . date("H:i") . " Hrs. del día " . date("d-M-Y") . "\r\n\r\n<br><br> Se cobró hasta el momento de asuntos: $" . number_format($asuma ,2) . " de la siguiente forma: \r\n\r\n<br><br> $pasuntos \r\n\r\n\r\n<br><br>" . "Se cobró hasta el momento en inmuebles: $" . number_format($suma ,2)      . "  de la siguiente forma: \r\n\r\n<br><br> $pagos \r\n\r\n\r\n<br><br> Se aplicó interes a los siguientes (Se agrega el IVA al interes en el c&aacute;lculo)<br>: \r\n\r\n<br><br>" . $mensaje . $datosapartado;
		//$enviocorreo->enviar("mizocotroco@hotmail.com", "Cierre del día manual", $mensaje);
	...
	{
		echo "<tr><td colspan=\"7\" align=\"center\"><input type =\"button\" value=\"Confirmar aplicaciones\" onClick=\"cargarSeccion('$dirscript','contenido','ver=1');\"  /></td></tr></table>";
	}
...
echo <<<fusuario1
<form action="cierremovil.php" method="POST">
<CENTER>
usuario<br>
