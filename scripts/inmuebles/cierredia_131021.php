...
*/	
		...
		//aplicar las penalizaciones correspondientes segun el cobro programado
			...
			if($ver==1){ //si confirmaron se hacen los procesos: $ver=1, es la confirmación del cierre
				...
				if(($row["concluido"]==false || is_null($row["concluido"])==true) && $fechanaturalpago!=0 && $row["activo"] == true)
				{
					...
					$sql2 .= $row["idcontra"] . "," . $row["idcobr"] . ",'" . $HoyFecha . "', '" . $row["fechanaturalpago"] . "', '" . $row["fechagracia"] . "'," . number_format($Apagar,2,".","") . "," . number_format($ivainteres,2,".","") . ",true," . $row["idusuarioh"] . ", 1, '" . $ProxVencimiento . "',false,1)";
	...
	if($ver==1)
	{
		...
		$enviocorreo->enviar("miguel@padillabujalil.com", "Cierre del día", $mensaje);
	}
	else
	{
