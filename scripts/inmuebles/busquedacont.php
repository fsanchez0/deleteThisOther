...
if($misesion->verifica_sesion()=="yes")
{
	...
	else
	{
		...
		{
			$idc=$row["idcontrato"];
			$direccion=$row["calle"] . "No." . $row["numeroext"] . " Int." . $row["numeroint"] . " Col." . $row["colonia"] . " Alc/Mun. ". $row["delmun"] . " C.P." . $row["cp"];
			$inquilino=$row["nombre"] . " " . $row["nombre2"] . " " . $row["apaterno"] . " " . $row["amaterno"] . "(Tels. " . $row["telinq"] . ", " . $row["telinm"] ;
			$html = "<tr>\n";
			$html .= "<td><a onClick=\"jabascript:cargarSeccion('$ruta/agregarc.php','contenido', 'paso=1&idc=$idc&efectivo=0&direccion=$direccion&inquilino=$inquilino');efect=0;\">" .  $idc . "</a></td>";
			$html .= "<td><a onClick=\"jabascript:cargarSeccion('$ruta/agregarc.php','contenido', 'paso=1&idc=$idc&efectivo=0&direccion=$direccion&inquilino=$inquilino');efect=0;\">" .  $row["nombre"] . " " . $row["nombre2"] . " " . $row["apaterno"] . " " . $row["amaterno"] . "</a></td>";
			$html .= "<td><a onClick=\"jabascript:cargarSeccion('$ruta/agregarc.php','contenido', 'paso=1&idc=$idc&efectivo=0&direccion=$direccion&inquilino=$inquilino');efect=0;\">" .  $direccion . "</a></td>";
