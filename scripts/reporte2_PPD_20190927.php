...
//+++++++++++++++++++++
	...
	$aa = $ftp->creararchivo($solicitud,$serie,$folio,$idfl,$filtro,'PPD');



	$ab = $ftp->enviar($idfl,1);
	if($ab==0)
	{
		echo "Error en el env&iacute;o";
	}
	$ac = $ftp->recoger($idfl,1);	
	...
	if(mysql_num_rows($operacioncfdi)>0){
		while($rowcfdi= mysql_fetch_array($operacioncfdi)){
			if($rowcfdi["tipofactura"]=='PPD'){
				$pago=true;
				$cfd->pagocfdi32class();
				$solicitud = $cfd->cadena_pagos($id,$cfd->lista,$row,$folio,$serie,$int);
			}
		}
	}else{		
		$solicitud = $cfd->cadena_factura($id,$cfd->lista,$row,$folio,$serie,$terceros,$int,0);	
	}
